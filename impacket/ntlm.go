...
NTLMSSP_AV_CHANNEL_BINDINGS = 0x0a
 ...
 type AV_PAIRS: struct {
     func (self TYPE) __init__(data = nil interface{}){
        self.fields = {}
        if data is not nil {
            self.fromString(data)

     func __setitem__(self,key,value interface{}){
     ...
     func (self TYPE) __delitem__(key interface{}){
        del self.fields[key]
 ...
 type NTLMAuthMixin: struct {
     func (self TYPE) get_os_version(){
        if self.os_version == '' {
            return nil
        } else  {
 ...
 type NTLMAuthNegotiate struct { // Structure, NTLMAuthMixin: (
        ('','"NTLMSSP\x00'),
         message_type uint32 // =1
         flags uint32 // 
         domain_len uint16 // -domain_name
         domain_max_len uint16 // -domain_name
         domain_offset uint32 // =0
         host_len uint16 // -host_name
         host_maxlen uint16 // -host_name
         host_offset uint32 // =0
        ('os_version',':'),
        ('host_name',':'),
        ('domain_name',':'))
     ...
     func (self TYPE) setWorkstation(workstation interface{}){
        self._workstation = workstation

     func (self TYPE) getWorkstation(){
 ...
 type NTLMAuthChallenge struct { // Structure: (
        ('','"NTLMSSP\x00'),
         message_type uint32 // =2
         domain_len uint16 // -domain_name
         domain_max_len uint16 // -domain_name
         domain_offset uint32 // =40
         flags uint32 // =0
         challenge [8]byte // 
         reserved [8]byte // =""
         TargetInfoFields_len uint16 // -TargetInfoFields
         TargetInfoFields_max_len uint16 // -TargetInfoFields
         TargetInfoFields_offset uint32 // 
    ...
    @staticmethod
     func checkVersion(flags interface{}){
        if flags is not nil {
     ...
     func (self TYPE) getData(){
        if self.TargetInfoFields"] is not nil and type(self["TargetInfoFields) is not bytes {
 ...
 type NTLMAuthChallengeResponse struct { // Structure, NTLMAuthMixin: (
        ('','"NTLMSSP\x00'),
         message_type uint32 // =3
         lanman_len uint16 // -lanman
         lanman_max_len uint16 // -lanman
         lanman_offset uint32 // 
         ntlm_len uint16 // -ntlm
         ntlm_max_len uint16 // -ntlm
         ntlm_offset uint32 // 
         domain_len uint16 // -domain_name
         domain_max_len uint16 // -domain_name
         domain_offset uint32 // 
         user_len uint16 // -user_name
         user_max_len uint16 // -user_name
         user_offset uint32 // 
         host_len uint16 // -host_name
         host_max_len uint16 // -host_name
         host_offset uint32 // 
         session_key_len uint16 // -session_key
         session_key_max_len uint16 // -session_key
         session_key_offset uint32 // 
         flags uint32 // 
        ('VersionLen','_-Version','self.checkVersion(self.flags)'), 
        ('Version',':=""'),
     ...
     func (self TYPE) __init__(username = "", password = "", challenge = "", lmhash = "", nthash = "", flags = 0 interface{}){
        Structure.__init__(self)
        self.session_key=''
 ...
 type ImpacketStructure struct { // Structure:
     func (self TYPE) set_parent(other interface{}){
        self.parent = other
 ...
 type ExtendedOrNotMessageSignature struct { // Structure:
     func (self TYPE) __init__(flags = 0, **kargs interface{}){
        if flags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY {
            self. self.extendedMessageSignature
        } else  {
            self. self.MessageSignature
        return Structure.__init__(self, **kargs)

 type NTLMMessageSignature struct { // ExtendedOrNotMessageSignature:
...
KNOWN_DES_INPUT = b"KGS!@//$%"
 ...
 func __expand_DES_key(key interface{}){
 ...
 func __DES_block(key, msg interface{}){
    cipher = DES.new(__expand_DES_key(key),DES.MODE_ECB)
    return cipher.encrypt(msg)

 func ntlmssp_DES_encrypt(key, challenge interface{}){
...
// High level functions to use NTLMSSP
 ...
 func getNTLMSSPType1(workstation='', domain='', signingRequired = false, use_ntlmv2 = USE_NTLMv2 interface{}){
 ...
 func getNTLMSSPType3(type1, type2, user, password, domain, lmhash = "", nthash = "", use_ntlmv2 = USE_NTLMv2 interface{}){

    // Safety check in case somebody sent password = nil.. That's not allowed. Setting it to '' and hope for the best.
    if password == nil {
        password = ""
    ...
    if encryptedRandomSessionKey is not nil { 
...
// NTLMv1 Algorithm
 ...
 func generateSessionKeyV1(password, lmhash, nthash interface{}){
    hash = MD4.new()
...
def computeResponseNTLMv1(flags, serverChallenge, clientChallenge, serverName, domain, user, password, lmhash='',
 ...
 func compute_lmhash(password interface{}){
 ...
 func NTOWFv1(password, lmhash = "", nthash='' interface{}){
    if nthash != '' {
       return nthash
    return compute_nthash(password)   

 func LMOWFv1(password, lmhash = "", nthash='' interface{}){
    if lmhash != '' {
       return lmhash
    return compute_lmhash(password)

 func compute_nthash(password interface{}){
    // This is done according to Samba's encryption specification (docs/html/ENCRYPTION.html)
    try:
 ...
 func get_ntlmv1_response(key, challenge interface{}){
...
// Crypto Stuff
 ...
 func MAC(flags, handle, signingKey, seqNum, message interface{}){
 ...
 func SEAL(flags, signingKey, sealingKey, messageToSign, messageToEncrypt, seqNum, handle interface{}){
 ...
 func SIGN(flags, signingKey, message, seqNum, handle interface{}){
   return MAC(flags, handle, signingKey, seqNum, message)

 func SIGNKEY(flags, randomSessionKey, mode = "Client" interface{}){
   if flags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY {
       if mode == 'Client' {
           md5 = hashlib.new("md5")
 ...
 func SEALKEY(flags, randomSessionKey, mode = "Client" interface{}){
   if flags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY {
 ...
 func generateEncryptedSessionKey(keyExchangeKey, exportedSessionKey interface{}){
 ...
 func KXKEY(flags, sessionBaseKey, lmChallengeResponse, serverChallenge, password, lmhash, nthash, use_ntlmv2 = USE_NTLMv2 interface{}){
   if use_ntlmv2 {
 ...
 func hmac_md5(key, data interface{}){
    import hmac
    h = hmac.new(key)
    h.update(data)
    return h.digest()

 func NTOWFv2( user, password, domain, hash = "" interface{}){
    if hash != '' {
       theHash = hash 
    } else  {
 ...
 func LMOWFv2( user, password, domain, lmhash = "" interface{}){
...
def computeResponseNTLMv2(flags, serverChallenge, clientChallenge, serverName, domain, user, password, lmhash='',
    ...
    if TEST_CASE is false {
        ...
        if av_pairs[NTLMSSP_AV_TIME] is not nil {
 ...
 type NTLM_HTTP struct { // object:
    // Parent  type for NTLM HTTP classes. struct {
    MSG_TYPE = nil

    @classmethod
     func get_instace(cls,msg_64 interface{}){
        msg = nil
        msg_type = 0
        if msg_64 != '' {
 ...
 type NTLM_HTTP_AuthRequired struct { // NTLM_HTTP:
    commonHdr = ()
 ...
 type NTLM_HTTP_AuthNegotiate struct { // NTLM_HTTP, NTLMAuthNegotiate:
    commonHdr = ()
    MSG_TYPE = 1
 ...
 type NTLM_HTTP_AuthChallengeResponse struct { // NTLM_HTTP, NTLMAuthChallengeResponse:
    commonHdr = ()
    MSG_TYPE = 3
