...
from impacket.ImpactPacket import ProtocolPacket
 ...
 type DNSType struct { // :
    ...
    @staticmethod
     func getTypeName(type interface{}){
 ...
 type DNSClass struct { // :
    ...
    @staticmethod
     func getClassName(type interface{}){
        for item, value in list(DNSClass.__dict__.items()):
            if value == type {
                return item

 type DNS struct { // ProtocolPacket:
    ...
    __OFFSETMASK   = int("00111111", 2)
     ...
     func (self TYPE) get_transaction_id(){
        'Get 16 bit message ID.'
        return self.header.get_word(0)
    
     func (self TYPE) set_transaction_id(value interface{}){
        'Set 16 bit message ID.'
        self.header.set_word(0, value)
    
     func (self TYPE) get_transaction_id_tcp(){
        'Get 16 bit message ID.'
        return self.header.get_word(2)
    
     func (self TYPE) set_transaction_id_tcp(value interface{}){
        'Set 16 bit message ID.'
     ...
     func (self TYPE) set_nscount(value interface{}){
     ...
     func (self TYPE) set_arcount(value interface{}){
     ...
     func (self TYPE) __get_questions(){
        ...
        for _ in range(qdcount): // number of questions
            ...
            q type = data[offset:offset+self.__CLASS_LEN] struct {
            offset  += self.__CLASS_LEN
            qtype  = struct.unpack("!H", qtype)[0]
            q type = struct.unpack struct { // "!H", qclass[0]
            aux.append((qname, qtype, qclass))
        return (aux, offset)

     func (self TYPE) get_questions_tcp(){
        'Get a list of the DNS Question.'
        return self.__get_questions_tcp()[0]

     func (self TYPE) __get_questions_tcp(){
        aux = []
        offset   = 2
     ...
     func (self TYPE) parseCompressedMessage(buf, offset=0 interface{}){
     ...
     func (self TYPE) get_answers(){
        return self.__get_answers()[0]
    
     func (self TYPE) get_authoritative(){
        return self.__get_authoritative()[0]
    
     func (self TYPE) get_additionals(){
        return self.__get_additionals()[0]
    
     func (self TYPE) __get_answers(){
     ...
     func (self TYPE) __get_authoritative(){
     ...
     func (self TYPE) __get_additionals(){
     ...
     func (self TYPE) __process_answer_structure(offset, num interface{}){
        aux  = []
        data = self.get_body_as_string()
        for _ in range(num):
            offset, qname = self.parseCompressedMessage(data, offset)
     ...
     func (self TYPE) __str__(){
        ...
        if qdcount > 0 {
            ...
            questions.reverse()
            while(questions):
                qname, qtype, q type = questions.pop struct { // 
     ...
     func (self TYPE) __get_questions_raw(){
     ...
     func (self TYPE) __get_answers_raw(){
     ...
     func (self TYPE) __get_authoritative_raw(){
     ...
     func (self TYPE) __get_additionals_raw(){
     ...
     func (self TYPE) add_answer(answer_raw interface{}){
        '''Add a raw answer'''
