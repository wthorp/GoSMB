 ...
 type Structure: struct {
    """ sublcasses can define commonHdr and/or structure.
        ...
        where format specifies how the data in the field will be converted to/from bytes (string)
         type is the class to use when unpacking ':' fields. struct {
    ...
    @classmethod
     func (self TYPE) fromFile(file interface{}){
        answer = self()
        answer.fromString(file.read(len(answer)))
        return answer

     func (self TYPE) setAlignment(alignment interface{}){
        self.alignment = alignment

     func (self TYPE) setData(data interface{}){
        self.data = data

     func (self TYPE) packField(fieldName, format = nil interface{}){
        if self.debug {
        ...
        if fieldName in self.fields {
            ...
            ans = self.pack(format, nil, field = fieldName)
     ...
     func (self TYPE) pack(format, data, field = nil interface{}){
        if self.debug {
            print("  pack( %s | %r | %s)" %  (format, data, field))

        if field {
            addressField = self.findAddressFieldFor(field)
            if (addressField is not nil) and (data == nil) {
                return b''
        ...
        if len(two) == 2 {
            try:
                ...
                if (two[1] in self.fields) and (self[two[1]] is not nil) {
     ...
     func (self TYPE) unpack(format, data, dataClassOrCode = b, field = nil interface{}){
        if self.debug {
            print("  unpack( %s | %r )" %  (format, data))

        if field {
            addressField = self.findAddressFieldFor(field)
            if addressField is not nil {
     ...
     func (self TYPE) calcPackSize(format, data, field = nil interface{}){
//        # print "  calcPackSize  %s:%r" %  (format, data)
     ...
     func (self TYPE) calcUnpackSize(format, data, field = nil interface{}){
        if self.debug {
     ...
     func (self TYPE) calcPackFieldSize(fieldName, format = nil interface{}){
        if format == nil {
            format = self.formatForField(fieldName)

        return self.calcPackSize(format, self[fieldName])

     func (self TYPE) formatForField(fieldName interface{}){
        for field in self.commonHdr+self.structure:
            if field[0] == fieldName {
                return field[1]
        raise Exception("Field %s not found" % fieldName)

     func (self TYPE) findAddressFieldFor(fieldName interface{}){
     ...
     func (self TYPE) findLengthFieldFor(fieldName interface{}){
        descriptor = "-%s" % fieldName
        l = len(descriptor)
        for field in self.commonHdr+self.structure:
            if field[1][-l:] == descriptor {
     ...
     func (self TYPE) zeroValue(format interface{}){
        two = format.split("*")
        if len(two) == 2 {
            if two[0].isdigit() {
 ...
 func hexdump(data, indent = "" interface{}){
    if data == nil {
        return
