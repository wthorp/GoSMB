...
// Standard Floor Assignments
 type EPMFloor struct { // Structure: (
         LHSByteCount uint16 // =0
        ('_ProtocolData','_-ProtocolData','self.LHSByteCount'),
        ('ProtocolData',':'),
         RHSByteCount uint16 // =0
        ('_RelatedData','_-RelatedData','self.RHSByteCount'),
        ('RelatedData',':'),
    } 

 type EPMRPCInterface struct { // EPMFloor: (
         LHSByteCount uint16 // =19
        ('InterfaceIdent','B=0x0d'),
         InterfaceUUID [6]byte // =""
         MajorVersion uint16 // =0
         RHSByteCount uint16 // =2
 ...
 type EPMRPCDataRepresentation struct { // EPMFloor: (
         LHSByteCount uint16 // =19
 ...
 type EPMProtocolIdentifier struct { // EPMFloor: (
         LHSByteCount uint16 // =1
        ('ProtIdentifier','B=0'),
         RHSByteCount uint16 // =2
         MinorVersion uint16 // =0
    }
 ...
 type EPMPipeName struct { // EPMFloor: (
         LHSByteCount uint16 // =1
        ('PipeIdentifier','B=15'),
         RHSByteCount uint16 // =len(PipeName)
        ('PipeName',':'),
    }

 type EPMHostName struct { // EPMFloor: (
         LHSByteCount uint16 // =1
        ('HostNameIdentifier','B=17'),
         RHSByteCount uint16 // =len(HostName)
        ('HostName',':'),
    }

 type EPMHostAddr struct { // EPMFloor: (
         LHSByteCount uint16 // =1
        ('HostAddressId','B=9'),
         RHSByteCount uint16 // =len(Ip4addr)
         Ip4addr [4]byte // =""
    }

 type EPMPortAddr struct { // EPMFloor: (
         LHSByteCount uint16 // =1
        ('PortIdentifier','B=7'),
         RHSByteCount uint16 // =2
        ('IpPort','>H=0'),
...
EPMFloor
 ...
 type EPMTower struct { // Structure: (
         NumberOfFloors uint16 // 
        ('Floors',':'),
    }
 ...
 type RPC_IF_ID struct { // NDRSTRUCT: (
        ('Uuid', UUID ),
        ('VersMajor', USHORT),
        ('VersMinor', USHORT),
    }

 type PRPC_IF_ID struct { // NDRPOINTER:
    referent = (
        ('Data', RPC_IF_ID),
    }

 type ept_lookup_handle_t struct { // NDRSTRUCT:  (
 ...
 type twr_t struct { // NDRSTRUCT: (
        ('tower_length', ULONG),
        ('tower_octet_string', NDRUniConformantArray),
    }

 type twr_p_t struct { // NDRPOINTER:
    referent = (
        ('Data', twr_t),
    }

 type octet_string_t struct { // NDRSTRUCT: (
        ('count', USHORT),
        ('value', LPBYTE),
    }

 type prot_and_addr_t struct { // NDRSTRUCT: (
        ('protocol_id', octet_string_t),
        ('address', octet_string_t),
    }

 type protocol_tower_t struct { // NDRSTRUCT: (
        ('count', USHORT),
        ('floors', prot_and_addr_t ),
    }

 type ept_entry_t struct { // NDRSTRUCT: (
        ('object',UUID),
        ('tower',twr_p_t),
        ('annotation', NDRUniVaryingArray),
    }

 type ept_entry_t_array struct { // NDRUniConformantVaryingArray:
    item = ept_entry_t

 type twr_p_t_array struct { // NDRUniConformantVaryingArray:
    item = twr_p_t
...
//###############################################################################
 ...
 type ept_lookup struct { // NDRCALL:
    opnum = 2 (
        ('inquiry_type',ULONG),
        ('object',PUUID),
        ('Ifid',PRPC_IF_ID),
 ...
 type ept_lookupResponse struct { // NDRCALL: (
        ('entry_handle',ept_lookup_handle_t),
        ('num_ents',ULONG),
 ...
 type ept_mapResponse struct { // NDRCALL: (
        ('entry_handle',ept_lookup_handle_t),
        ('num_towers',ULONG),
...
//###############################################################################
 ...
 func hept_lookup(destHost, inquiry_type = RPC_C_EP_ALL_ELTS, objectUUID = NULL, ifId = NULL, vers_option = RPC_C_VERS_ALL,  entry_handle = ept_lookup_handle_t(), max_ents = 499, dce = nil interface{}){
    if dce == nil {
 ...
 func hept_map(destHost, remoteIf, dataRepresentation = uuidtup_to_bin(('8a885d04-1ceb-11c9-9fe8-08002b104860', '2.0')), protocol = "ncacn_np", dce=nil interface{}){
 ...
 func PrintStringBinding(floors, serverAddr = "0.0.0.0" interface{}){
    tmp_address = ""
