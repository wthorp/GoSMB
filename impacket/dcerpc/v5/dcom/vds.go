...
// 2.2.2.1.3.1 VDS_SERVICE_PROP
 type VDS_SERVICE_PROP struct { // NDRSTRUCT: (
        ('pwszVersion',LPWSTR),
        ('ulFlags',ULONG),
    }
...
// 2.2.2.7.1.1 VDS_PROVIDER_TYPE
 type VDS_PROVIDER_TYPE struct { // NDRENUM:
...
// 2.2.2.7.2.1 VDS_PROVIDER_PROP
 type VDS_PROVIDER_PROP struct { // NDRSTRUCT: (
        ('id',VDS_OBJECT_ID),
        ('pwszName',LPWSTR),
        ('guidVersionId',GUID),
        ('pwszVersion',LPWSTR),
...
// 3.4.5.2.5.1 IVdsServiceInitialization::Initialize (Opnum 3)
 type IVdsServiceInitialization_Initialize struct { // DCOMCALL:
    opnum = 3 (
       ('pwszMachineName', LPWSTR),
    }

 type IVdsServiceInitialization_InitializeResponse struct { // DCOMANSWER: (
       ('ErrorCode', error_status_t),
    }

// 3.4.5.2.4.1 IVdsService::IsServiceReady (Opnum 3)
 type IVdsService_IsServiceReady struct { // DCOMCALL:
    opnum = 3 (
    }

 type IVdsService_IsServiceReadyResponse struct { // DCOMANSWER: (
       ('ErrorCode', error_status_t),
    }

// 3.4.5.2.4.2 IVdsService::WaitForServiceReady (Opnum 4)
 type IVdsService_WaitForServiceReady struct { // DCOMCALL:
    opnum = 4 (
    }

 type IVdsService_WaitForServiceReadyResponse struct { // DCOMANSWER: (
       ('ErrorCode', error_status_t),
    }

// 3.4.5.2.4.3 IVdsService::GetProperties (Opnum 5)
 type IVdsService_GetProperties struct { // DCOMCALL:
    opnum = 5 (
    }

 type IVdsService_GetPropertiesResponse struct { // DCOMANSWER: (
       ('pServiceProp', VDS_SERVICE_PROP),
       ('ErrorCode', error_status_t),
    }

// 3.4.5.2.4.4 IVdsService::QueryProviders (Opnum 6)
 type IVdsService_QueryProviders struct { // DCOMCALL:
    opnum = 6 (
        ('masks', DWORD),
    }

 type IVdsService_QueryProvidersResponse struct { // DCOMANSWER: (
...
// 3.4.5.2.1.1 IEnumVdsObject::Next (Opnum 3)
 type IEnumVdsObject_Next struct { // DCOMCALL:
    opnum = 3 (
       ('celt', ULONG),
    }

 type IEnumVdsObject_NextResponse struct { // DCOMANSWER: (
       ('ppObjectArray', OBJECT_ARRAY),
       ('pcFetched', ULONG),
       ('ErrorCode', error_status_t),
    }
// 3.4.5.2.14.1 IVdsProvider::GetProperties (Opnum 3)
 type IVdsProvider_GetProperties struct { // DCOMCALL:
    opnum = 3 (
    }

 type IVdsProvider_GetPropertiesResponse struct { // DCOMANSWER: (
...
//###############################################################################
 type IEnumVdsObject struct { // IRemUnknown2:
     func (self TYPE) Next(celt=0xffff interface{}){
 ...
 type IVdsProvider struct { // IRemUnknown2:
 ...
 type IVdsService struct { // IRemUnknown2:
     func (self TYPE) __init__(interface interface{}){
        IRemUnknown2.__init__(self, interface)

     func (self TYPE) IsServiceReady(){
     ...
     func (self TYPE) WaitForServiceReady(){
     ...
     func (self TYPE) QueryProviders(masks interface{}){
