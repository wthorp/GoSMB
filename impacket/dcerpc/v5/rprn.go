...
STRING_HANDLE = LPWSTR
 type PSTRING_HANDLE struct { // NDRPOINTER:
    referent = (
        ('Data', STRING_HANDLE),
    }
...
// 2.2.1.1.4 PRINTER_HANDLE
 type PRINTER_HANDLE struct { // NDRSTRUCT:  (
         Data [0]byte // =b""
    }
...
// 2.2.1.2.1 DEVMODE_CONTAINER
 ...
 type DEVMODE_CONTAINER struct { // NDRSTRUCT:  (
        ('cbBuf',DWORD),
        ('pDevMode',PBYTE_ARRAY),
    }

// 2.2.1.11.1 SPLCLIENT_INFO_1
 type SPLCLIENT_INFO_1 struct { // NDRSTRUCT:  (
        ('dwSize',DWORD),
        ('pMachineName',LPWSTR),
        ('pUserName',LPWSTR),
        ('dwBuildNum',DWORD),
 ...
 type PSPLCLIENT_INFO_1 struct { // NDRPOINTER:
    referent = (
        ('Data', SPLCLIENT_INFO_1),
    }

// 2.2.1.11.2 SPLCLIENT_INFO_2
 type SPLCLIENT_INFO_2 struct { // NDRSTRUCT:  (
        ('notUsed',ULONGLONG),
    }

 type PSPLCLIENT_INFO_2 struct { // NDRPOINTER:
    referent = (
        ('Data', SPLCLIENT_INFO_2),
    }
// 2.2.1.11.3 SPLCLIENT_INFO_3
 type SPLCLIENT_INFO_3 struct { // NDRSTRUCT:  (
        ('cbSize',UINT),
        ('dwFlags',DWORD),
        ('dwFlags',DWORD),
        ('pMachineName',LPWSTR),
 ...
 type PSPLCLIENT_INFO_3 struct { // NDRPOINTER:
    referent = (
        ('Data', SPLCLIENT_INFO_3),
    }
// 2.2.1.2.14 SPLCLIENT_CONTAINER
 type CLIENT_INFO_UNION struct { // NDRUNION:
    commonHdr = (
        ('tag', ULONG),
    }
    union = {
 ...
 type SPLCLIENT_CONTAINER struct { // NDRSTRUCT:  (
        ('Level',DWORD),
...
// 2.2.1.13.2 RPC_V2_NOTIFY_OPTIONS_TYPE
 ...
 type RPC_V2_NOTIFY_OPTIONS_TYPE struct { // NDRSTRUCT:  (
        ('Type',USHORT),
        ('Reserved0',USHORT),
        ('Reserved1',DWORD),
        ('Reserved2',DWORD),
 ...
 type PRPC_V2_NOTIFY_OPTIONS_TYPE_ARRAY struct { // NDRPOINTER:
    referent = (
        ('Data', RPC_V2_NOTIFY_OPTIONS_TYPE),
    }

// 2.2.1.13.1 RPC_V2_NOTIFY_OPTIONS
 type RPC_V2_NOTIFY_OPTIONS struct { // NDRSTRUCT:  (
        ('Version',DWORD),
        ('Reserved',DWORD),
        ('Count',DWORD),
 ...
 type PRPC_V2_NOTIFY_OPTIONS struct { // NDRPOINTER:
    referent = (
...
// 3.1.4.2.1 RpcEnumPrinters (Opnum 0)
 type RpcEnumPrinters struct { // NDRCALL:
    opnum = 0 (
       ('Flags', DWORD),
       ('Name', STRING_HANDLE),
       ('Level', DWORD),
 ...
 type RpcEnumPrintersResponse struct { // NDRCALL: (
       ('pPrinterEnum', PBYTE_ARRAY),
       ('pcbNeeded', DWORD),
       ('pcReturned', DWORD),
       ('ErrorCode', ULONG),
...
// 3.1.4.2.2 RpcOpenPrinter (Opnum 1)
 type RpcOpenPrinter struct { // NDRCALL:
    opnum = 1 (
 ...
 type RpcOpenPrinterResponse struct { // NDRCALL: (
       ('pHandle', PRINTER_HANDLE),
       ('ErrorCode', ULONG),
    }

// 3.1.4.2.9 RpcClosePrinter (Opnum 29)
 type RpcClosePrinter struct { // NDRCALL:
    opnum = 29 (
       ('phPrinter', PRINTER_HANDLE),
    }

 type RpcClosePrinterResponse struct { // NDRCALL: (
       ('phPrinter', PRINTER_HANDLE),
       ('ErrorCode', ULONG),
    }

// 3.1.4.10.4 RpcRemoteFindFirstPrinterChangeNotificationEx (Opnum 65)
 type RpcRemoteFindFirstPrinterChangeNotificationEx struct { // NDRCALL:
    opnum = 65 (
 ...
 type RpcRemoteFindFirstPrinterChangeNotificationExResponse struct { // NDRCALL: (
       ('ErrorCode', ULONG),
    }

// 3.1.4.2.14 RpcOpenPrinterEx (Opnum 69)
 type RpcOpenPrinterEx struct { // NDRCALL:
    opnum = 69 (
       ('pPrinterName', STRING_HANDLE),
       ('pDatatype', LPWSTR),
       ('pDevModeContainer', DEVMODE_CONTAINER),
 ...
 type RpcOpenPrinterExResponse struct { // NDRCALL: (
       ('pHandle', PRINTER_HANDLE),
       ('ErrorCode', ULONG),
    }
...
//###############################################################################
 ...
 func hRpcOpenPrinter(dce, printerName, pDatatype = NULL, pDevModeContainer = NULL, accessRequired = SERVER_READ interface{}){
    """
 ...
 func hRpcClosePrinter(dce, phPrinter interface{}){
    """
...
def hRpcRemoteFindFirstPrinterChangeNotificationEx(dce, hPrinter, fdwFlags, fdwOptions=0, pszLocalMachine=NULL,
 ...
 func hRpcEnumPrinters(dce, flags, name = NULL, level = 1 interface{}){
    """
