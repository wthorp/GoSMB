...
rpc_status_codes = {
 ...
 type DCERPCException struct { // Exception:
    """
    ...
    """
     ...
     func get_packet( self  interface{}){
        return self.packet

     func __str__( self  interface{}){
        key = self.error_code
        if self.error_string is not nil {
...
// Context Item
 type CtxItem struct { // Structure: (
         ContextID uint16 // =0
        ('TransItems','B=0'),
        ('Pad','B=0'),
         AbstractSyntax [0]byte // =""
         TransferSyntax [0]byte // =""
    }

 type CtxItemResult struct { // Structure: (
         Result uint16 // =0
         Reason uint16 // =0
         TransferSyntax [0]byte // =""
    }

 type SEC_TRAILER struct { // Structure:
    commonHdr = (
        ('auth_type', 'B=10'),
        ('auth_level','B=0'),
        ('auth_pad_len','B=0'),
        ('auth_rsvrd','B=0'),
         auth_ctx_id uint32 // =747920
    }

 type MSRPCHeader struct { // Structure:
    _SIZE = 16
    commonHdr = ( 
        ('ver_major','B=5'),                              // 0
        ('ver_minor','B=0'),                              // 1
        ('type','B=0'),                                   // 2
        ('flags','B=0'),                                  // 3
         representation uint32 // =0x10                     // 4
         frag_len uint16 // =self._SIZE+len(auth_data)+(16 if (self.flags & 0x80) > 0 else 0)+len(pduData)+len(pad)+len(sec_trailer)  // 8
         auth_len uint16 // =len(auth_data)                 // 10
         call_id uint32 // =1                               // 12    <-- Common up to here (including this)
    } ( 
 ...
 type MSRPCRequestHeader struct { // MSRPCHeader:
    _SIZE = 24
    commonHdr = MSRPCHeader.commonHdr + ( 
         alloc_hint uint32 // =0                            // 16
         ctx_id uint16 // =0                                // 20
         op_num uint16 // =0                                // 22
 ...
 type MSRPCRespHeader struct { // MSRPCHeader:
    _SIZE = 24
    commonHdr = MSRPCHeader.commonHdr + ( 
         alloc_hint uint32 // =0                          // 16   
         ctx_id uint16 // =0                              // 20
         cancel_count byte // =0                        // 22
         padding byte // =0                             // 23
    }

     func (self TYPE) __init__(aBuffer = nil, alignment = 0 interface{}){
        MSRPCHeader.__init__(self, aBuffer, alignment)
        if aBuffer == nil {
            self.type = MSRPC_RESPONSE
            self.ctx_id = 0

 type MSRPCBind struct { // Structure:
    _CTX_ITEM_LEN = len(CtxItem()) ( 
         max_tfrag uint16 // =4280
         max_rfrag uint16 // =4280
         assoc_group uint32 // =0
        ('ctx_num','B=0'),
        ('Reserved','B=0'),
         Reserved2 uint16 // =0
     ...
     func (self TYPE) addCtxItem(item interface{}){
 ...
 type MSRPCBindAck struct { // MSRPCHeader:
    _SIZE = 26 // Up to SecondaryAddr
    _CTX_ITEM_LEN = len(CtxItemResult()) ( 
         max_tfrag uint16 // =0
         max_rfrag uint16 // =0
         assoc_group uint32 // =0
         SecondaryAddrLen uint16 // &SecondaryAddr 
     ...
     func (self TYPE) getCtxItems(){
        return self.__ctx_items

     func getCtxItem(self,index interface{}){
 ...
 type MSRPCBindNak struct { // Structure: ( 
         RejectedReason uint16 // =0
        ('SupportedVersions',':'),
    }
     func (self TYPE) __init__(data = nil, alignment = 0 interface{}){
        Structure.__init__(self,data,alignment)
 ...
 type DCERPC: struct {
    ...
    transfer_syntax =  NDRSyntax
     ...
     func __init__(self,transport interface{}){
        self._transport = transport
        self.set_ctx_id(0)
        self._max_user_frag = nil
        self.set_default_max_fragment_size()
        self._ctx = nil

     func (self TYPE) get_rpc_transport(){
        return self._transport

     func (self TYPE) set_ctx_id(ctx_id interface{}){
        self._ctx = ctx_id
     ...
     func (self TYPE) set_max_fragment_size(fragment_size interface{}){
     ...
     func (self TYPE) send(data interface{}){
        raise RuntimeError ("virtual method. Not implemented in subclass")

     func (self TYPE) recv(){
        raise RuntimeError ("virtual method. Not implemented in subclass")

     func (self TYPE) alter_ctx(newUID, bogus_binds='' interface{}){
        raise RuntimeError ("virtual method. Not implemented in subclass")

     func (self TYPE) set_credentials(username, password, domain='', lmhash='', nthash='', aesKey='', TGT=nil, TGS=nil interface{}){
        pass

     func (self TYPE) set_auth_level(auth_level interface{}){
        pass

     func (self TYPE) set_auth_type(auth_type, callback=nil interface{}){
        pass

     func (self TYPE) get_idempotent(){
        return 0

     func (self TYPE) set_idempotent(flag interface{}){
        pass

     func (self TYPE) call(function, body, uuid=nil interface{}){
     ...
     func (self TYPE) request(request, uuid=nil, checkError=true interface{}){
 ...
 type DCERPC_v4 struct { // DCERPC:
    pass

 type DCERPC_v5 struct { // DCERPC:
     func (self TYPE) __init__(transport interface{}){
        DCERPC.__init__(self, transport)
        self.__auth_level = RPC_C_AUTHN_LEVEL_NONE
        self.__auth_type = RPC_C_AUTHN_WINNT
        self.__auth_type_callback = nil
     ...
     func (self TYPE) set_session_key(session_key interface{}){
     ...
     func (self TYPE) set_max_tfrag(size interface{}){
     ...
     func (self TYPE) bind(iface_uuid, alter = 0, bogus_binds = 0, transfer_syntax = ('8a885d04-1ceb-11c9-9fe8-08002b104860', '2.0') interface{}){
        bind = MSRPCBind()
        ...
        if self.__auth_level != RPC_C_AUTHN_LEVEL_NONE {
            if (self.__username == nil) or (self.__password == nil) {
     ...
     func (self TYPE) _transport_send(rpc_packet, forceWriteAndx = 0, forceRecv = 0 interface{}){
     ...
     func (self TYPE) alter_ctx(newUID, bogus_binds = 0 interface{}){
 ...
 type DCERPC_RawCall struct { // MSRPCRequestHeader:
     func (self TYPE) __init__(op_num, data = b'', uuid=nil interface{}){
     ...
     func (self TYPE) setData(data interface{}){
        self.pduData = data

// 2.2.6 Type Serialization Version 1
 type CommonHeader struct { // NDRSTRUCT: (
        ('Version', UCHAR),
        ('Endianness', UCHAR),
 ...
 type PrivateHeader struct { // NDRSTRUCT: (
 ...
 type TypeSerialization1 struct { // NDRSTRUCT:
    commonHdr = (
 ...
 type DCERPCServer struct { // Thread:
    """
    ...
    If you want to implement a DCE Interface Server, use this  type as the base class struct {
    """
     ...
     func (self TYPE) addCallbacks(ifaceUUID, secondaryAddr, callbacks interface{}){
        """
     ...
     func (self TYPE) setListenPort(portNum interface{}){
        self._listenPort = portNum
        self._sock = socket.socket()
        self._sock.bind((self._listenAddress,self._listenPort))

     func (self TYPE) getListenPort(){
     ...
     func bind(self,packet, bind interface{}){
        // Standard NDR Representation
     ...
     func processRequest(self,data interface{}){
