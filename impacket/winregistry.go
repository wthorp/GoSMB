...
// Structs
 type REG_REGF struct { // Structure: (
        ('Magic','"regf'),
         Unknown uint32 // =0
         Unknown2 uint32 // =0
         lastChange uint64 // =0
         ...
         0 uint32 // =0
         11 uint32 // =0
         OffsetFirstRecord uint32 // =0
         DataSize uint32 // =0
         1111 uint32 // =0
         Name [8]byte // =""
         Remaining1 [1]byte // =b""
         CheckSum uint32 // =0xffffffff // Sum of all DWORDs from 0x0 to 0x1FB
         Remaining2 [5]byte // =b""
    }

 type REG_HBIN struct { // Structure: (
        ('Magic','"hbin'),
         OffsetFirstHBin uint32 // =0
         OffsetNextHBin uint32 // =0
         BlockSize uint32 // =0
    }

 type REG_HBINBLOCK struct { // Structure: (
        ('DataBlockSize','<l=0'),
        ('_Data','_-Data','self.DataBlockSize*(-1)-4'),
        ('Data',':'),
    }

 type REG_NK struct { // Structure: (
        ('Magic','"nk'),
         Type uint16 // =0
         lastChange uint64 // =0
         Unknown uint32 // =0
        ...
        ('OffsetClassName','<l=0'),
         UnUsed [0]byte // =b""
         NameLength uint16 // =0
         ClassNameLength uint16 // =0
        ('_KeyName','_-KeyName','self.NameLength'),
        ('KeyName',':'),
    }

 type REG_VK struct { // Structure: (
        ('Magic','"vk'),
         NameLength uint16 // =0
        ('DataLen','<l=0'),
         OffsetData uint32 // =0
 ...
 type REG_LF struct { // Structure: (
        ('Magic','"lf'),
         NumKeys uint16 // =0
        ('HashRecords',':'),
    }

 type REG_LH struct { // Structure: (
        ('Magic','"lh'),
         NumKeys uint16 // =0
        ('HashRecords',':'),
    }

 type REG_RI struct { // Structure: (
        ('Magic','"ri'),
         NumKeys uint16 // =0
        ('HashRecords',':'),
    }

 type REG_SK struct { // Structure: (
        ('Magic','"sk'),
         UnUsed uint16 // =0
        ('OffsetPreviousSk','<l=0'),
        ('OffsetNextSk','<l=0'),
         UsageCounter uint32 // =0
         SizeSk uint32 // =0
        ('Data',':'),
    }

 type REG_HASH struct { // Structure: (
         OffsetNk uint32 // =0
         KeyName [4]byte // =b""
    }
...
StructMappings = {b'nk': REG_NK,
 ...
 type Registry: struct {
     func (self TYPE) __init__(hive, isRemote = false interface{}){
        self.__hive = hive
        if isRemote is true {
            self.fd = self.__hive
            self.__hive.open()
     ...
     func (self TYPE) __del__(){
        self.close()

     func (self TYPE) __findRootKey(){
        self.fd.seek(0,0)
        data = self.fd.read(4096)
        while len(data) > 0:
            try:
     ...
     func (self TYPE) __getBlock(offset interface{}){
     ...
     func (self TYPE) __getValueBlocks(offset, count interface{}){
        valueList = []
        res = []
        self.fd.seek(4096+offset,0)
     ...
     func (self TYPE) __getData(offset, count interface{}){
        self.fd.seek(4096+offset, 0)
        return self.fd.read(count)[4:]

     func __processDataBlocks(self,data interface{}){
        res = []
        while len(data) > 0:
     ...
     func (self TYPE) __getValueData(rec interface{}){
     ...
     func (self TYPE) __getLhHash(key interface{}){
        res = 0
        for bb in key.upper():
            res *= 37
            res += ord(bb)
     ...
     func (self TYPE) __compareHash(magic, hashData, key interface{}){
        if magic == 'lf' {
     ...
     func (self TYPE) __findSubKey(parentKey, subKey interface{}){
     ...
     func (self TYPE) __walkSubNodes(rec interface{}){
     ...
     func (self TYPE) walk(parentKey interface{}){
        key = self.findKey(parentKey)

        if key == nil or key["OffsetSubKeyLf"] < 0 {
            return
     ...
     func (self TYPE) findKey(key interface{}){
     ...
     func (self TYPE) printValue(valueType, valueData interface{}){
     ...
     func (self TYPE) enumKey(parentKey interface{}){
        res = []
     ...
     func enumValues(self,key interface{}){
        // If we're here.. we have a valid NK record for the key
