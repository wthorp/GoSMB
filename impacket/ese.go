...
ColumnTypeSize = {
    ...
    JET_coltypBinary       : nil,
    JET_coltypText         : nil, 
    JET_coltypLongBinary   : nil,
    JET_coltypLongText     : nil,
    JET_coltypSLV          : nil,
...
TABLE_CURSOR = {
 ...
 type ESENT_JET_SIGNATURE struct { // Structure: (
         Random uint32 // =0
         CreationTime uint64 // =0
         NetBiosName [6]byte // =b""
    }

 type ESENT_DB_HEADER struct { // Structure: (
         CheckSum uint32 // =0
        ...
        ('DBSignature',':',ESENT_JET_SIGNATURE),
         DBState uint32 // =0
         ConsistentPosition uint64 // =0
         ConsistentTime uint64 // =0
         AttachTime uint64 // =0
         AttachPosition uint64 // =0
         DetachTime uint64 // =0
         DetachPosition uint64 // =0
        ('LogSignature',':',ESENT_JET_SIGNATURE),
         Unknown uint32 // =0
         PreviousBackup [4]byte // =b""
         PreviousIncBackup [4]byte // =b""
         CurrentFullBackup [4]byte // =b""
         ShadowingDisables uint32 // =0
         LastObjectID uint32 // =0
         WindowsMajorVersion uint32 // =0
         WindowsMinorVersion uint32 // =0
         WindowsBuildNumber uint32 // =0
         WindowsServicePackNumber uint32 // =0
         FileFormatRevision uint32 // =0
         PageSize uint32 // =0
         RepairCount uint32 // =0
         RepairTime uint64 // =0
         Unknown2 [8]byte // =b""
         ScrubTime uint64 // =0
         RequiredLog uint64 // =0
         UpgradeExchangeFormat uint32 // =0
         UpgradeFreePages uint32 // =0
         UpgradeSpaceMapPages uint32 // =0
         CurrentShadowBackup [4]byte // =b""
         CreationFileFormatVersion uint32 // =0
         CreationFileFormatRevision uint32 // =0
         Unknown3 [6]byte // =b""
         OldRepairCount uint32 // =0
         ECCCount uint32 // =0
         LastECCTime uint64 // =0
         OldECCFixSuccessCount uint32 // =0
         ECCFixErrorCount uint32 // =0
         LastECCFixErrorTime uint64 // =0
         OldECCFixErrorCount uint32 // =0
         BadCheckSumErrorCount uint32 // =0
         LastBadCheckSumTime uint64 // =0
         OldCheckSumErrorCount uint32 // =0
         CommittedLog uint32 // =0
         PreviousShadowCopy [4]byte // =b""
         PreviousDifferentialBackup [4]byte // =b""
         Unknown4 [0]byte // =b""
         NLSMajorVersion uint32 // =0
         NLSMinorVersion uint32 // =0
         Unknown5 [8]byte // =b""
         UnknownFlags uint32 // =0
    }

 type ESENT_PAGE_HEADER struct { // Structure:
    structure_2003_SP0 = (
         CheckSum uint32 // =0
         PageNumber uint32 // =0
    }
    structure_0x620_0x0b = (
         CheckSum uint32 // =0
         ECCCheckSum uint32 // =0
    }
    structure_win7 = (
         CheckSum uint64 // =0
    }
    common = (
         LastModificationTime uint64 // =0
         PreviousPageNumber uint32 // =0
         NextPageNumber uint32 // =0
         FatherDataPage uint32 // =0
         AvailableDataSize uint16 // =0
         AvailableUncommittedDataSize uint16 // =0
         FirstAvailableDataOffset uint16 // =0
         FirstAvailablePageTag uint16 // =0
         PageFlags uint32 // =0
    }
    extended_win7 = (
         ExtendedCheckSum1 uint64 // =0
         ExtendedCheckSum2 uint64 // =0
         ExtendedCheckSum3 uint64 // =0
         PageNumber uint64 // =0
         Unknown uint64 // =0
    }
     func (self TYPE) __init__(version, revision, pageSize=8192, data=nil interface{}){
        if (version < 0x620) or (version == 0x620 and revision < 0x0b) {
            // For sure the old format
            self. self.structure_2003_SP0 + self.common
        elif version == 0x620 and revision < 0x11 {
            // Exchange 2003 SP1 and Windows Vista and later
            self. self.structure_0x620_0x0b + self.common
        } else  {
            // Windows 7 and later
            self. self.structure_win7 + self.common
            if pageSize > 8192 {
                self.structure += self.extended_win7

        Structure.__init__(self,data)

 type ESENT_ROOT_HEADER struct { // Structure: (
         InitialNumberOfPages uint32 // =0
         ParentFatherDataPage uint32 // =0
         ExtentSpace uint32 // =0
         SpaceTreePageNumber uint32 // =0
    }

 type ESENT_BRANCH_HEADER struct { // Structure: (
        ('CommonPageKey',':'),
    }

 type ESENT_BRANCH_ENTRY struct { // Structure:
    common = (
         CommonPageKeySize uint16 // =0
    } (
         LocalPageKeySize uint16 // =0
        ('_LocalPageKey','_-LocalPageKey','self.LocalPageKeySize'),
        ('LocalPageKey',':'),
         ChildPageNumber uint32 // =0
    }
     func (self TYPE) __init__(flags, data=nil interface{}){
        if flags & TAG_COMMON > 0 {
            // Include the common header
            self. self.common + self.structure
        Structure.__init__(self,data)

 type ESENT_LEAF_HEADER struct { // Structure: (
        ('CommonPageKey',':'),
    }

 type ESENT_LEAF_ENTRY struct { // Structure:
    common = (
         CommonPageKeySize uint16 // =0
    } (
         LocalPageKeySize uint16 // =0
 ...
 type ESENT_SPACE_TREE_HEADER struct { // Structure: (
         Unknown uint64 // =0
    }

 type ESENT_SPACE_TREE_ENTRY struct { // Structure: (
         PageKeySize uint16 // =0
         LastPageNumber uint32 // =0
         NumberOfPages uint32 // =0
    }

 type ESENT_INDEX_ENTRY struct { // Structure: (
        ('RecordPageKey',':'),
    }

 type ESENT_DATA_DEFINITION_HEADER struct { // Structure: (
         LastFixedSize byte // =0
         LastVariableDataType byte // =0
         VariableSizeOffset uint16 // =0
    }

 type ESENT_CATALOG_DATA_DEFINITION_ENTRY struct { // Structure:
    fixed = (
         FatherDataPageID uint32 // =0
         Type uint16 // =0
         Identifier uint32 // =0
    }
    ...
    column_stuff = (
         ColumnType uint32 // =0
         SpaceUsage uint32 // =0
         ColumnFlags uint32 // =0
         CodePage uint32 // =0
    }

    other = (
         FatherDataPageNumber uint32 // =0
    }
...
//         InitialNumberOfPages uint32 // =0
    ...
    common = (
//         RootFlag byte // =0
//         RecordOffset uint16 // =0
//         LCMapFlags uint32 // =0
//         KeyMost uint16 // =0
        ('Trailing',':'),
    }

     func __init__(self,data interface{}){
...
//        i += 16
 ...
 type ESENT_PAGE: struct {
     func (self TYPE) __init__(db, data=nil interface{}){
        self.__DBHeader = db
        self.data = data
        self.record = nil
        if data is not nil {
     ...
     func (self TYPE) getTag(tagNum interface{}){
 ...
 type ESENT_DB: struct {
     func (self TYPE) __init__(fileName, pageSize = 8192, isRemote = false interface{}){
        self.__fileName = fileName
        self.__pageSize = pageSize
        self.__DB = nil
        self.__DBHeader = nil
        self.__totalPages = nil
        self.__tables = OrderedDict()
        self.__currentTable = nil
        self.__isRemote = isRemote
        self.mountDB()

     func (self TYPE) mountDB(){
     ...
     func (self TYPE) __addItem(entry interface{}){
     ...
     func __parseItemName(self,entry interface{}){
        dataDefinitionHeader = ESENT_DATA_DEFINITION_HEADER(entry["EntryData"])
     ...
     func (self TYPE) __addLongValue(entry interface{}){
        dataDefinitionHeader = ESENT_DATA_DEFINITION_HEADER(entry["EntryData"])
     ...
     func (self TYPE) parsePage(page interface{}){
        // Print the leaf/branch tags
     ...
     func (self TYPE) parseCatalog(pageNum interface{}){
     ...
     func (self TYPE) readHeader(){
        LOG.debug("Reading Boot Sector for %s" % self.__volumeName)

     func (self TYPE) getPage(pageNum interface{}){
     ...
     func (self TYPE) openTable(tableName interface{}){
     ...
     func (self TYPE) __getNextTag(cursor interface{}){
     ...
     func (self TYPE) getNextRow(cursor interface{}){
        cursor["CurrentTag"] += 1
     ...
     func (self TYPE) __tagToRecord(cursor, tag interface{}){
