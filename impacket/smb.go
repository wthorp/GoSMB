...
FILE_OVERWRITE_IF               = 0x00000005
 ...
 func strerror(errclass, errcode interface{}){
    if err type == 0x01 { struct {
        return 'OS error', ERRDOS.get(errcode, 'Unknown error')
    elif err type == 0x02 { struct {
        return 'Server error', ERRSRV.get(errcode, 'Unknown error')
    elif err type == 0x03 { struct {
        return 'Hardware error', ERRHRD.get(errcode, 'Unknown error')
    // This is not a standard error  type for SMB struct {
    //elif err type == 0x80 { struct {
    //    return 'Browse error', ERRBROWSE.get(errcode, 'Unknown error')
    elif err type == 0xff { struct {
 ...
 type SessionError struct { // Exception:
  ...
  0xFFFF: ("ERRnosupport", "Function not supported.")
    ...
    error_classes = { 0: ("SUCCESS", {}),
     ...
     func __init__( self, error_string, error_class, error_code, nt_status = 0, packet=0 interface{}){
        Exception.__init__(self, error_string)
        self.nt_status = nt_status
        self._args = error_string
        if nt_status {
           self.error_ type = 0 struct {
     ...
     func get_error_class( self  interface{}){
        return self.error_class

     func get_error_code( self  interface{}){
        return self.error_code

     func (self TYPE) get_error_packet(){
        return self.packet

     func __str__( self  interface{}){
        error_ type = SessionError.error_classes.get struct { //  self.error_class, nil 
        if not error_class {
            error_code_str = self.error_code
            error_class_str = self.error_class
        } else  {
            error_class_str = error_class[0]
            error_code = error_class[1].get( self.error_code, nil )
            if not error_code {
                error_code_str = self.error_code
            } else  {
...
// currently supported by pysmb
 type UnsupportedFeature struct { // Exception:
    pass

// Contains information about a SMB shared device/service
 type SharedDevice: struct {
     func (self TYPE) __init__(name, share_type, comment interface{}){
        self.__name = name
...
// Contains information about the shared file/directory
 type SharedFile: struct {
     func (self TYPE) __init__(ctime, atime, mtime, filesize, allocsize, attribs, shortname, longname interface{}){
        self.__ctime = ctime
        self.__atime = atime
        self.__mtime = mtime
     ...
     func (self TYPE) get_ctime(){
        return self.__ctime

     func (self TYPE) get_ctime_epoch(){
        return self.__convert_smbtime(self.__ctime)

     func (self TYPE) get_mtime(){
        return self.__mtime

     func (self TYPE) get_mtime_epoch(){
        return self.__convert_smbtime(self.__mtime)

     func (self TYPE) get_atime(){
        return self.__atime

     func (self TYPE) get_atime_epoch(){
        return self.__convert_smbtime(self.__atime)

     func (self TYPE) get_filesize(){
        return self.__filesize

     func (self TYPE) get_allocsize(){
        return self.__allocsize

     func (self TYPE) get_attributes(){
        return self.__attribs

     func (self TYPE) is_archive(){
        return self.__attribs & ATTR_ARCHIVE

     func (self TYPE) is_compressed(){
        return self.__attribs & ATTR_COMPRESSED

     func (self TYPE) is_normal(){
        return self.__attribs & ATTR_NORMAL

     func (self TYPE) is_hidden(){
        return self.__attribs & ATTR_HIDDEN

     func (self TYPE) is_readonly(){
        return self.__attribs & ATTR_READONLY

     func (self TYPE) is_temporary(){
        return self.__attribs & ATTR_TEMPORARY

     func (self TYPE) is_directory(){
        return self.__attribs & ATTR_DIRECTORY

     func (self TYPE) is_system(){
        return self.__attribs & ATTR_SYSTEM

     func (self TYPE) get_shortname(){
        return self.__shortname

     func (self TYPE) get_longname(){
    ...
    @staticmethod
     func __convert_smbtime(t interface{}){
        x = t >> 32
        y = t & 0xffffffff
...
// Contain information about a SMB machine
 type SMBMachine: struct {
     func (self TYPE) __init__(nbname, nbt_type, comment interface{}){
 ...
 type SMBDomain: struct {
     func (self TYPE) __init__(nbgroup, domain_type, master_browser interface{}){
...
// Represents a SMB Packet
 type NewSMBPacket struct { // Structure: (
        ('Signature', '"\xffSMB'),
        ('Command','B=0'),
        ('ErrorClass','B=0'),
        ('_reserved','B=0'),
     ...
     func (self TYPE) __init__(**kargs interface{}){
     ...
     func (self TYPE) addCommand(command interface{}){
        if len(self.Data) == 0 {
     ...
     func (self TYPE) isMoreProcessingRequired(){
        return self.ErrorClass"] == 0x16 and self["ErrorCode == 0xc000

     func (self TYPE) isValidAnswer(cmd interface{}){
        // this was inside a loop reading more from the net (with recv_packet(nil))
        if self.Command == cmd {
 ...
 type AsciiOrUnicodeStructure struct { // Structure:
    UnicodeStructure = ()
    AsciiStructure   = ()
     func (self TYPE) __init__(flags = 0, **kargs interface{}){
        if flags & SMB.FLAGS2_UNICODE {
            self. self.UnicodeStructure
        } else  {
            self. self.AsciiStructure
        Structure.__init__(self, **kargs)

 type SMBCommand_Parameters struct { // Structure:
    pass

 type SMBAndXCommand_Parameters struct { // Structure:
    commonHdr = (
        ('AndXCommand','B=0xff'),
        ('_reserved','B=0'),
         AndXOffset uint16 // =0
    } (       // default structure, overridden by subclasses
        ('Data',':=""'),
    }
...
// SMB_QUERY_FS_ATTRIBUTE_INFO
 type SMBQueryFsAttributeInfo struct { // Structure: (
         FileSystemAttributes uint32 // 
         MaxFilenNameLengthInBytes uint32 // 
         LengthOfFileSystemName uint32 // -FileSystemName
        ('FileSystemName',':'),
    }

 type SMBQueryFsInfoVolume struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         ulVolSerialNbr uint32 // =0xABCDEFAA
         cCharCount byte // -VolumeLabel
    }
    AsciiStructure = (
        ('VolumeLabel','z'),
    }
...
// FILE_FS_SIZE_INFORMATION
 type FileFsSizeInformation struct { // Structure: (
         TotalAllocationUnits int64 // =148529400
         AvailableAllocationUnits int64 // =14851044
         SectorsPerAllocationUnit uint32 // =2
         BytesPerSector uint32 // =512
    }

// SMB_QUERY_FS_SIZE_INFO
 type SMBQueryFsSizeInfo struct { // Structure: (
         TotalAllocationUnits int64 // =148529400
         TotalFreeAllocationUnits int64 // =14851044
         SectorsPerAllocationUnit uint32 // =2
         BytesPerSector uint32 // =512
    }
// FILE_FS_FULL_SIZE_INFORMATION
 type SMBFileFsFullSizeInformation struct { // Structure: (
         TotalAllocationUnits int64 // =148529400
         CallerAvailableAllocationUnits int64 // =148529400
         ActualAvailableAllocationUnits int64 // =148529400
         SectorsPerAllocationUnit uint32 // =15
        ('BytesPerSector','<L=512')
    }
// SMB_QUERY_FS_VOLUME_INFO
 type SMBQueryFsVolumeInfo struct { // Structure: (
         VolumeCreationTime int64 // 
         SerialNumber uint32 // =0xABCDEFAA
         VolumeLabelSize uint32 // =len(VolumeLabel)
         Reserved uint16 // =0x10
        ('VolumeLabel',':')
    }
// SMB_FIND_FILE_BOTH_DIRECTORY_INFO level
 type SMBFindFileBothDirectoryInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         ...
         ExtFileAttributes uint32 // =0
    }
    AsciiStructure = (
         FileNameLength uint32 // -FileName','len(FileName)
         EaSize uint32 // =0
         ShortNameLength byte // =0
         Reserved byte // =0
         ShortName [4]byte // 
        ('FileName',':'),
    }
    UnicodeStructure = (
         FileNameLength uint32 // -FileName','len(FileName)*2
         EaSize uint32 // =0
         ShortNameLength byte // =0
         Reserved byte // =0
         ShortName [4]byte // 
...
// SMB_FIND_FILE_ID_FULL_DIRECTORY_INFO level
 type SMBFindFileIdFullDirectoryInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         FileIndex uint32 // =0
         CreationTime int64 // 
...
// SMB_FIND_FILE_ID_BOTH_DIRECTORY_INFO level
 type SMBFindFileIdBothDirectoryInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         FileIndex uint32 // =0
         CreationTime int64 // 
...
// SMB_FIND_FILE_DIRECTORY_INFO level
 type SMBFindFileDirectoryInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         FileIndex uint32 // =0
         CreationTime int64 // 
...
// SMB_FIND_FILE_NAMES_INFO level
 type SMBFindFileNamesInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         FileIndex uint32 // =0
    }
...
// SMB_FIND_FILE_FULL_DIRECTORY_INFO level
 type SMBFindFileFullDirectoryInfo struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         FileIndex uint32 // =0
         CreationTime int64 // 
...
// SMB_FIND_INFO_STANDARD level
 type SMBFindInfoStandard struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         ResumeKey uint32 // =0xff
         CreationDate uint16 // =0
         CreationTime uint16 // =0
         LastAccessDate uint16 // =0
         LastAccessTime uint16 // =0
         LastWriteDate uint16 // =0
         LastWriteTime uint16 // =0
         EaSize uint32 // 
         AllocationSize uint32 // =1
         ExtFileAttributes uint16 // =0
    }
    AsciiStructure = (
         FileNameLength byte // -FileName','len(FileName)
        ('FileName','z'),
    }
    UnicodeStructure = (
         FileNameLength byte // -FileName','len(FileName)*2
        ('FileName',':'),
    }

// SET_FILE_INFORMATION structures
// SMB_SET_FILE_DISPOSITION_INFO
 type SMBSetFileDispositionInfo struct { // Structure: (
         DeletePending byte // 
    }

// SMB_SET_FILE_BASIC_INFO
 type SMBSetFileBasicInfo struct { // Structure: (
         CreationTime int64 // 
         LastAccessTime int64 // 
         LastWriteTime int64 // 
         ChangeTime int64 // 
         ExtFileAttributes uint16 // 
         Reserved uint32 // 
    }

// FILE_STREAM_INFORMATION
 type SMBFileStreamInformation struct { // Structure:
    commonHdr = (
         NextEntryOffset uint32 // =0
         StreamNameLength uint32 // =0
         StreamSize int64 // =0
         StreamAllocationSize int64 // =0
        ('StreamName',':=""'),
    }

// FILE_NETWORK_OPEN_INFORMATION
 type SMBFileNetworkOpenInfo struct { // Structure: (
...
// SMB_SET_FILE_END_OF_FILE_INFO
 type SMBSetFileEndOfFileInfo struct { // Structure: (
         EndOfFile int64 // 
    }

// TRANS2_FIND_NEXT2
 type SMBFindNext2_Parameters struct { // AsciiOrUnicodeStructure:
     commonHdr = (
          SID uint16 // 
          SearchCount uint16 // 
 ...
 type SMBFindNext2Response_Parameters struct { // Structure: (
          SearchCount uint16 // 
          EndOfSearch uint16 // =1
          EaErrorOffset uint16 // =0
          LastNameOffset uint16 // =0
     }

 type SMBFindNext2_Data struct { // Structure: (
...
// TRANS2_FIND_FIRST2 
 type SMBFindFirst2Response_Parameters struct { // Structure: (
          SID uint16 // 
          SearchCount uint16 // 
          EndOfSearch uint16 // =1
          EaErrorOffset uint16 // =0
 ...
 type SMBFindFirst2_Parameters struct { // AsciiOrUnicodeStructure:
     commonHdr = (
          SearchAttributes uint16 // 
          SearchCount uint16 // 
          Flags uint16 // 
          InformationLevel uint16 // 
          SearchStorageType uint32 // 
     }
     AsciiStructure = (
         ('FileName','z'),
     }
 ...
 type SMBFindFirst2_Data struct { // Structure: (
         ('GetExtendedAttributesListLength','_-GetExtendedAttributesList', 'self.GetExtendedAttributesListLength'),
         ('GetExtendedAttributesList',':'),
     }

// TRANS2_SET_PATH_INFORMATION
 type SMBSetPathInformation_Parameters struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         InformationLevel uint16 // 
         Reserved uint32 // 
    }
 ...
 type SMBSetPathInformationResponse_Parameters struct { // Structure: (
         EaErrorOffset uint16 // =0
    }

// TRANS2_SET_FILE_INFORMATION
 type SMBSetFileInformation_Parameters struct { // Structure: (
         FID uint16 // 
         InformationLevel uint16 // 
         Reserved uint16 // 
    }

 type SMBSetFileInformationResponse_Parameters struct { // Structure: (
         EaErrorOffset uint16 // =0
    }

// TRANS2_QUERY_FILE_INFORMATION
 type SMBQueryFileInformation_Parameters struct { // Structure: (
         FID uint16 // 
         InformationLevel uint16 // 
    }

 type SMBQueryFileInformationResponse_Parameters struct { // Structure: (
         EaErrorOffset uint16 // =0
    }

 type SMBQueryFileInformation_Data struct { // Structure: (
        ('GetExtendedAttributeList',':'),
    }

// TRANS2_QUERY_PATH_INFORMATION
 type SMBQueryPathInformationResponse_Parameters struct { // Structure: (
         EaErrorOffset uint16 // =0
    }

 type SMBQueryPathInformation_Parameters struct { // AsciiOrUnicodeStructure:
    commonHdr = (
         InformationLevel uint16 // 
         Reserved uint32 // =0
    }
 ...
 type SMBQueryPathInformation_Data struct { // Structure: (
        ('GetExtendedAttributeList',':'),
    }


// SMB_QUERY_FILE_EA_INFO
 type SMBQueryFileEaInfo struct { // Structure: (
         EaSize uint32 // =0
    }

// SMB_QUERY_FILE_BASIC_INFO
 type SMBQueryFileBasicInfo struct { // Structure: (
         CreationTime int64 // 
         LastAccessTime int64 // 
         LastWriteTime int64 // 
         LastChangeTime int64 // 
...
// SMB_QUERY_FILE_STANDARD_INFO
 type SMBQueryFileStandardInfo struct { // Structure: (
...
// SMB_QUERY_FILE_ALL_INFO
 type SMBQueryFileAllInfo struct { // Structure: (
         CreationTime int64 // 
         LastAccessTime int64 // 
         LastWriteTime int64 // 
         LastChangeTime int64 // 
...
// \PIPE\LANMAN NetShareEnum
 type SMBNetShareEnum struct { // Structure: (
         RAPOpcode uint16 // =0
        ('ParamDesc','z'),
        ('DataDesc','z'),
         InfoLevel uint16 // 
         ReceiveBufferSize uint16 // 
    }

 type SMBNetShareEnumResponse struct { // Structure: (
         Status uint16 // =0
         Convert uint16 // =0
         EntriesReturned uint16 // 
         EntriesAvailable uint16 // 
    }

 type NetShareInfo1 struct { // Structure: (
         NetworkName [3]byte // 
         Pad byte // =0
         Type uint16 // =0
         RemarkOffsetLow uint16 // =0
         RemarkOffsetHigh uint16 // =0
    }

// \PIPE\LANMAN NetServerGetInfo
 type SMBNetServerGetInfoResponse struct { // Structure: (
         Status uint16 // =0
         Convert uint16 // =0
         TotalBytesAvailable uint16 // 
    }

 type SMBNetServerInfo1 struct { // Structure:
    // Level 1 Response (
         ServerName [6]byte // 
        ('MajorVersion','B=5'),
        ('MinorVersion','B=0'),
         ServerType uint32 // =3
         ServerCommentLow uint16 // =0
         ServerCommentHigh uint16 // =0
    }

// \PIPE\LANMAN NetShareGetInfo
 type SMBNetShareGetInfo struct { // Structure: (
         RAPOpcode uint16 // =0
        ('ParamDesc','z'),
        ('DataDesc','z'),
        ('ShareName','z'),
 ...
 type SMBNetShareGetInfoResponse struct { // Structure: (
         Status uint16 // =0
         Convert uint16 // =0
         TotalBytesAvailable uint16 // 
    }
...
//############ Security Features
 type SecurityFeatures struct { // Structure: (
         Key uint32 // =0
         CID uint16 // =0
...
//############ SMB_COM_QUERY_INFORMATION2 (0x23)
 type SMBQueryInformation2_Parameters struct { // Structure: (
         Fid uint16 // 
    }

 type SMBQueryInformation2Response_Parameters struct { // Structure: (
         CreateDate uint16 // 
         CreationTime uint16 // 
...
//############ SMB_COM_SESSION_SETUP_ANDX (0x73)
 type SMBSessionSetupAndX_Parameters struct { // SMBAndXCommand_Parameters: (
         MaxBuffer uint16 // 
         MaxMpxCount uint16 // 
         VCNumber uint16 // 
         SessionKey uint32 // 
         AnsiPwdLength uint16 // 
         UnicodePwdLength uint16 // 
         _reserved uint32 // =0
         Capabilities uint32 // 
    }

 type SMBSessionSetupAndX_Extended_Parameters struct { // SMBAndXCommand_Parameters: (
 ...
 type SMBSessionSetupAndX_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
 ...
 type SMBSessionSetupAndX_Extended_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
 ...
 type SMBSessionSetupAndXResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         Action uint16 // 
    }

 type SMBSessionSetupAndX_Extended_Response_Parameters struct { // SMBAndXCommand_Parameters: (
         Action uint16 // =0
         SecurityBlobLength uint16 // 
    }

 type SMBSessionSetupAndXResponse_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('NativeOS','z=""'),
        ('NativeLanMan','z=""'),
        ('PrimaryDomain','z=""'),
 ...
 type SMBSessionSetupAndX_Extended_Response_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('SecurityBlobLength','_-SecurityBlob','self.SecurityBlobLength'),
        ('SecurityBlob',':'),
        ('NativeOS','z=""'),
    ...
    UnicodeStructure = (
     ...
     func (self TYPE) getData(){
        if self.= self.UnicodeStructure {
            if len(str(self.SecurityBlob)) % 2 == 0 {
                self.Pad = "\x00"
        return AsciiOrUnicodeStructure.getData(self)

//############ SMB_COM_TREE_CONNECT (0x70)
 type SMBTreeConnect_Parameters struct { // SMBCommand_Parameters: (
    }

 type SMBTreeConnect_Data struct { // SMBCommand_Parameters: (
        ('PathFormat','"\x04'),
        ('Path','z'),
        ('PasswordFormat','"\x04'),
        ('Password','z'),
...
//############ SMB_COM_TREE_CONNECT_ANDX (0x75)
 type SMBTreeConnectAndX_Parameters struct { // SMBAndXCommand_Parameters: (
         Flags uint16 // =0
         PasswordLength uint16 // 
    }

 type SMBTreeConnectAndXResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         OptionalSupport uint16 // =0
    }

 type SMBTreeConnectAndXExtendedResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         OptionalSupport uint16 // =1
         MaximalShareAccessRights uint32 // =0x1fffff
         GuestMaximalShareAccessRights uint32 // =0x1fffff
    }

 type SMBTreeConnectAndX_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
 ...
 type SMBTreeConnectAndXResponse_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('Service','z'),
...
//############ SMB_COM_NT_CREATE_ANDX (0xA2)
 type SMBNtCreateAndX_Parameters struct { // SMBAndXCommand_Parameters: (
        ('_reserved', 'B=0'),
         FileNameLength uint16 //      // NameLength
         CreateFlags uint32 //         // Flags
         RootFid uint32 // =0          // RootDirectoryFID
         AccessMask uint32 //          // DesiredAccess
         AllocationSizeLo uint32 // =0 // AllocationSize
         AllocationSizeHi uint32 // =0
         FileAttributes uint32 // =0   // ExtFileAttributes
         ShareAccess uint32 // =3      //
         Disposition uint32 // =1      // CreateDisposition
         CreateOptions uint32 //       // CreateOptions
         Impersonation uint32 // =2
        ('SecurityFlags','B=3'),
    }

 type SMBNtCreateAndXResponse_Parameters struct { // SMBAndXCommand_Parameters:
 ...
 type SMBNtCreateAndXExtendedResponse_Parameters struct { // SMBAndXCommand_Parameters:
    // [MS-SMB] Extended response description (
        ...
        ('IsDirectory','B'),
         ...
         FileIdLow uint32 // =0
         FileIdHigh uint32 // =0
         MaximalAccessRights uint32 // =0x12019b
         GuestMaximalAccessRights uint32 // =0x120089
    }

 type SMBNtCreateAndX_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('FileName','z'),
    }
    UnicodeStructure = (
...
//############ SMB_COM_OPEN_ANDX (0xD2)
 type SMBOpenAndX_Parameters struct { // SMBAndXCommand_Parameters: (
         Flags uint16 // =0
         DesiredAccess uint16 // =0
         SearchAttributes uint16 // =0
         FileAttributes uint16 // =0
         CreationTime uint32 // =0
         OpenMode uint16 // =1        // SMB_O_OPEN = 1
         AllocationSize uint32 // =0
         Reserved [8]byte // =""
    }

 type SMBOpenAndX_Data struct { // SMBNtCreateAndX_Data:
    pass

 type SMBOpenAndXResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         Fid uint16 // =0
         FileAttributes uint16 // =0
         LastWriten uint32 // =0
         FileSize uint32 // =0
         GrantedAccess uint16 // =0
         FileType uint16 // =0
         IPCState uint16 // =0
         Action uint16 // =0
         ServerFid uint32 // =0
...
//############ SMB_COM_WRITE (0x0B)
 type SMBWrite_Parameters struct { // SMBCommand_Parameters: (
         Fid uint16 // 
         Count uint16 // 
         Offset uint32 // 
         Remaining uint16 // 
 ...
 type SMBWriteResponse_Parameters struct { // SMBCommand_Parameters: (
         Count uint16 // 
    }

 type SMBWrite_Data struct { // Structure: (
         BufferFormat byte // =1
         DataLength uint16 // -Data
        ('Data',':'),
    }


//############ SMB_COM_WRITE_ANDX (0x2F)
 type SMBWriteAndX_Parameters struct { // SMBAndXCommand_Parameters: (
         Fid uint16 // =0
         Offset uint32 // =0
         _reserved uint32 // =0xff
         WriteMode uint16 // =8
         Remaining uint16 // =0
         DataLength_Hi uint16 // =0
         DataLength uint16 // =0
         DataOffset uint16 // =0
         HighOffset uint32 // =0
    }

 type SMBWriteAndX_Data_Short struct { // Structure: (
 ...
 type SMBWriteAndX_Data struct { // Structure: (
 ...
 type SMBWriteAndX_Parameters_Short struct { // SMBAndXCommand_Parameters: (
         Fid uint16 // 
         Offset uint32 // 
         _reserved uint32 // =0xff
         WriteMode uint16 // =8
 ...
 type SMBWriteAndXResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         Count uint16 // 
         Available uint16 // 
         Reserved uint32 // =0
    }
...
//############ SMB_COM_WRITE_RAW (0x1D)
 type SMBWriteRaw_Parameters struct { // SMBCommand_Parameters: (
         Fid uint16 // 
         Count uint16 // 
         _reserved uint16 // =0
         Offset uint32 // 
...
//############ SMB_COM_READ (0x0A)
 type SMBRead_Parameters struct { // SMBCommand_Parameters: (
         Fid uint16 // 
         Count uint16 // 
         Offset uint32 // 
 ...
 type SMBReadResponse_Parameters struct { // Structure: (
         Count uint16 // =0
         _reserved [8]byte // =""
    }

 type SMBReadResponse_Data struct { // Structure: (
         BufferFormat byte // =0x1
         DataLength uint16 // -Data
        ('Data',':'),
    }

//############ SMB_COM_READ_RAW (0x1A)
 type SMBReadRaw_Parameters struct { // SMBCommand_Parameters: (
         Fid uint16 // 
         Offset uint32 // 
         MaxCount uint16 // 
         MinCount uint16 // =MaxCount
         Timeout uint32 // =0
         _reserved uint16 // =0
    }

//############ SMB_COM_NT_TRANSACT  (0xA0)
 type SMBNTTransaction_Parameters struct { // SMBCommand_Parameters: (
         MaxSetupCount byte // =0
         Reserved1 uint16 // =0
         TotalParameterCount uint32 // 
         TotalDataCount uint32 // 
         MaxParameterCount uint32 // =1024
         MaxDataCount uint32 // =65504
         ParameterCount uint32 // 
         ParameterOffset uint32 // 
         DataCount uint32 // 
         DataOffset uint32 // 
         SetupCount byte // =len(Setup)//2
         Function uint16 // =0
        ('SetupLength','_-Setup','SetupCount*2'),
        ('Setup',':'),
    }

 type SMBNTTransactionResponse_Parameters struct { // SMBCommand_Parameters: (
         Reserved1 [3]byte // =""
         TotalParameterCount uint32 // 
         TotalDataCount uint32 // 
         ParameterCount uint32 // 
         ParameterOffset uint32 // 
         ParameterDisplacement uint32 // =0
         DataCount uint32 // 
         DataOffset uint32 // 
         DataDisplacement uint32 // =0
         SetupCount byte // =0
        ('SetupLength','_-Setup','SetupCount*2'),
        ('Setup',':'),
    }

 type SMBNTTransaction_Data struct { // Structure: (
 ...
 type SMBNTTransactionResponse_Data struct { // Structure: (
        ('Pad1Length','_-Pad1','self.Pad1Length'),
        ('Pad1',':'),
...
//############ SMB_COM_TRANSACTION2_SECONDARY (0x33)
 type SMBTransaction2Secondary_Parameters struct { // SMBCommand_Parameters: (
         ...
         DataCount uint16 // 
         DataOffset uint16 // 
         DataDisplacement uint16 // =0
         FID uint16 // 
    }

 type SMBTransaction2Secondary_Data struct { // Structure: (
        ('Pad1Length','_-Pad1','self.Pad1Length'),
        ('Pad1',':'),
        ('Trans_ParametersLength','_-Trans_Parameters','self.Trans_ParametersLength'),
        ('Trans_Parameters',':'),
...
//############ SMB_COM_TRANSACTION2 (0x32)
 ...
 type SMBTransaction2_Parameters struct { // SMBCommand_Parameters: (
         TotalParameterCount uint16 // 
         TotalDataCount uint16 // 
         MaxParameterCount uint16 // =1024
         MaxDataCount uint16 // =65504
         MaxSetupCount byte // =0
         Reserved1 byte // =0
         Flags uint16 // =0
         Timeout uint32 // =0
 ...
 type SMBTransaction2Response_Parameters struct { // SMBCommand_Parameters: (
         TotalParameterCount uint16 // 
         TotalDataCount uint16 // 
         Reserved1 uint16 // =0
         ParameterCount uint16 // 
         ParameterOffset uint16 // 
         ParameterDisplacement uint16 // =0
         DataCount uint16 // 
         DataOffset uint16 // 
         DataDisplacement uint16 // =0
         SetupCount byte // =0
 ...
 type SMBTransaction2_Data struct { // Structure: (
//        ('NameLength','_-Name','1'),
//        ('Name',':'),
        ('Pad1Length','_-Pad1','self.Pad1Length'),
        ('Pad1',':'),
 ...
 type SMBTransaction2Response_Data struct { // Structure: (
        ('Pad1Length','_-Pad1','self.Pad1Length'),
        ('Pad1',':'),
        ('Trans_ParametersLength','_-Trans_Parameters','self.Trans_ParametersLength'),
        ('Trans_Parameters',':'),
...
//############ SMB_COM_QUERY_INFORMATION (0x08)
 ...
 type SMBQueryInformation_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('BufferFormat','B=4'),
        ('FileName','z'),
    }
 ...
 type SMBQueryInformationResponse_Parameters struct { // Structure: (
...
//############ SMB_COM_TRANSACTION (0x25)
 type SMBTransaction_Parameters struct { // SMBCommand_Parameters: (
         TotalParameterCount uint16 // 
         TotalDataCount uint16 // 
         MaxParameterCount uint16 // =1024
         MaxDataCount uint16 // =65504
 ...
 type SMBTransactionResponse_Parameters struct { // SMBCommand_Parameters: (
         TotalParameterCount uint16 // 
         TotalDataCount uint16 // 
         Reserved1 uint16 // =0
         ParameterCount uint16 // 
...
// the instances where this structure is used on the client side
 type SMBTransaction_SData struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('Name','z'),
 ...
 type SMBTransaction_Data struct { // Structure: (
        ('NameLength','_-Name'),
        ('Name',':'),
        ('Trans_ParametersLength','_-Trans_Parameters'),
        ('Trans_Parameters',':'),
 ...
 type SMBTransactionResponse_Data struct { // Structure: (
        ('Trans_ParametersLength','_-Trans_Parameters'),
        ('Trans_Parameters',':'),
        ('Trans_DataLength','_-Trans_Data'),
        ('Trans_Data',':'),
...
//############ SMB_COM_READ_ANDX (0x2E)
 type SMBReadAndX_Parameters struct { // SMBAndXCommand_Parameters: (
         Fid uint16 // 
         Offset uint32 // 
         MaxCount uint16 // 
         MinCount uint16 // =MaxCount
         _reserved uint32 // =0x0
         Remaining uint16 // =MaxCount
         HighOffset uint32 // =0
    }

 type SMBReadAndX_Parameters2 struct { // SMBAndXCommand_Parameters: (
         Fid uint16 // 
         Offset uint32 // 
         MaxCount uint16 // 
         MinCount uint16 // =MaxCount
         _reserved uint32 // =0xffffffff
         Remaining uint16 // =MaxCount
    }

 type SMBReadAndXResponse_Parameters struct { // SMBAndXCommand_Parameters: (
         Remaining uint16 // =0
         DataMode uint16 // =0
         _reserved uint16 // =0
         DataCount uint16 // 
...
//############ SMB_COM_ECHO (0x2B)
 type SMBEcho_Data struct { // Structure: (
        ('Data',':'),
    }

 type SMBEcho_Parameters struct { // Structure: (
         EchoCount uint16 // 
    }

 type SMBEchoResponse_Data struct { // Structure: (
        ('Data',':'),
    }

 type SMBEchoResponse_Parameters struct { // Structure: (
         SequenceNumber uint16 // =1
    }

//############ SMB_COM_QUERY_INFORMATION_DISK (0x80)
 type SMBQueryInformationDiskResponse_Parameters struct { // Structure: (
         TotalUnits uint16 // 
         BlocksPerUnit uint16 // 
         BlockSize uint16 // 
         FreeUnits uint16 // 
         Reserved uint16 // =0
    }
...
//############ SMB_COM_LOGOFF_ANDX (0x74)
 type SMBLogOffAndX struct { // SMBAndXCommand_Parameters:
    strucure = ()

//############ SMB_COM_CLOSE (0x04)
 type SMBClose_Parameters struct { // SMBCommand_Parameters: (
         FID uint16 // 
         Time uint32 // =0
   }

//############ SMB_COM_FLUSH (0x05)
 type SMBFlush_Parameters struct { // SMBCommand_Parameters: (
         FID uint16 // 
   }

//############ SMB_COM_CREATE_DIRECTORY (0x00)
 type SMBCreateDirectory_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
         BufferFormat byte // =4
        ('DirectoryName','z'),
    }
...
//############ SMB_COM_DELETE (0x06)
 type SMBDelete_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
         BufferFormat byte // =4
        ('FileName','z'),
    }
 ...
 type SMBDelete_Parameters struct { // Structure: (
         SearchAttributes uint16 // 
    }

//############ SMB_COM_DELETE_DIRECTORY (0x01)
 type SMBDeleteDirectory_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
         BufferFormat byte // =4
        ('DirectoryName','z'),
    }
...
//############ SMB_COM_CHECK_DIRECTORY (0x10)
 type SMBCheckDirectory_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
         BufferFormat byte // =4
        ('DirectoryName','z'),
    }
...
//############ SMB_COM_RENAME (0x07)
 type SMBRename_Parameters struct { // SMBCommand_Parameters: (
         SearchAttributes uint16 // 
    }

 type SMBRename_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
         BufferFormat1 byte // =4
        ('OldFileName','z'),
         BufferFormat2 byte // =4
        ('NewFileName','z'),
    }
    UnicodeStructure = (
         BufferFormat1 byte // =4
...
//############ SMB_COM_OPEN (0x02)
 type SMBOpen_Parameters struct { // SMBCommand_Parameters: (
         DesiredAccess uint16 // =0
         SearchAttributes uint16 // =0
    }

 type SMBOpen_Data struct { // AsciiOrUnicodeStructure:
    AsciiStructure = (
        ('FileNameFormat','"\x04'),
        ('FileName','z'),
    }
 ...
 type SMBOpenResponse_Parameters struct { // SMBCommand_Parameters: (
         Fid uint16 // =0
         FileAttributes uint16 // =0
         LastWriten uint32 // =0
         FileSize uint32 // =0
...
//############ EXTENDED SECURITY CLASSES
 type SMBExtended_Security_Parameters struct { // Structure: (
         DialectIndex uint16 // 
         SecurityMode byte // 
         MaxMpxCount uint16 // 
         MaxNumberVcs uint16 // 
 ...
 type SMBExtended_Security_Data struct { // Structure: (
         ServerGUID [6]byte // 
        ('SecurityBlob',':'),
    }

 type SMBNTLMDialect_Parameters struct { // Structure: (
         DialectIndex uint16 // 
         SecurityMode byte // 
         MaxMpxCount uint16 // 
         MaxNumberVcs uint16 // 
 ...
 type SMBNTLMDialect_Data struct { // Structure: (
...
// For some reason on an old Linux this field is not present, we have to check this out. There must be a flag stating this.
     ...
     func __init__(self,data = nil, alignment = 0 interface{}){
         Structure.__init__(self,data,alignment)
         //self.ChallengeLength=8

     func fromString(self,data interface{}){
 ...
 type SMB: struct {
    ...
    def __init__(self, remote_name, remote_host, my_name=nil, host_type=nmb.TYPE_SERVER, sess_port=445, timeout=nil,
                 UDP=0, session=nil, negPacket=nil):
        // The uid attribute will be set when the client calls the login() method
        self._uid = 0
        self.__server_name = ""
        self.__client_name = ""
        self.__server_os = ""
        self.__server_os_major = nil
        self.__server_os_minor = nil
        self.__server_os_build = nil
        ...
        self._dialects_parameters = nil
        self._dialects_data = nil
    ...
    @staticmethod
     ...
     func (self TYPE) getKerberos(){
        return self._doKerberos

     func (self TYPE) get_remote_name(){
        return self.__remote_name

     func (self TYPE) set_remote_name(name interface{}){
        self.__remote_name = name
        return true

     func (self TYPE) get_remote_host(){
        return self.__remote_host

     func (self TYPE) get_flags(){
        return self.__flags1, self.__flags2

     func (self TYPE) set_flags(flags1=nil, flags2=nil interface{}){
        if flags1 is not nil {
           self.__flags1 = flags1
        if flags2 is not nil {
           self.__flags2 = flags2

     func (self TYPE) set_timeout(timeout interface{}){
        prev_timeout = self.__timeout
        self.__timeout = timeout
        return prev_timeout

     func (self TYPE) get_timeout(){
        return self.__timeout

    @contextmanager
     func (self TYPE) use_timeout(timeout interface{}){
     ...
     func (self TYPE) get_session(){
        return self._sess

     func (self TYPE) get_tid(){
        return self.tid

     func (self TYPE) get_fid(){
        return self.fid
     ...
     func (self TYPE) close_session(){
        if self._sess {
            self._sess.close()
            self._sess = nil

     func (self TYPE) recvSMB(){
        r = self._sess.recv_packet(self.__timeout)
        return NewSMBPacket(data = r.get_trailer())

    @staticmethod
     func __decode_trans(params, data interface{}){
    ...
    // TODO: Move this to NewSMBPacket, it belongs there
     func (self TYPE) signSMB(packet, signingSessionKey, signingChallengeResponse interface{}){
     ...
     func (self TYPE) checkSignSMB(packet, signingSessionKey, signingChallengeResponse interface{}){
        // Let's check
     ...
     func sendSMB(self,smb interface{}){
        smb["Uid"] = self._uid
    ...
    @staticmethod
     func isValidAnswer(s, cmd interface{}){
        while 1:
            if s.rawData() {
     ...
     func (self TYPE) neg_session(extended_security = true, negPacket = nil interface{}){
         func parsePacket(smb interface{}){
            ...
            if smb.isValidAnswer(SMB.SMB_COM_NEGOTIATE) {
                ...
                // If not, let's try the old way
                    ...
                    if self._dialects_data["ServerName"] is not nil {
     ...
     func (self TYPE) tree_connect(path, password = "", service = SERVICE_ANY interface{}){
     ...
     func (self TYPE) get_uid(){
        return self._uid

     func (self TYPE) set_uid(uid interface{}){
        self._uid = uid

     func (self TYPE) tree_connect_andx(path, password = nil, service = SERVICE_ANY, smb_packet=nil interface{}){
        if password {
            // Password is only encrypted if the server passed us an "encryption" during protocol dialect
            if self._dialects_parameters["ChallengeLength"] > 0 {
                // this code is untested
    ...
    @staticmethod
     ...
     func (self TYPE) get_server_name(){
        //return self._dialects_data["ServerName"]
        return self.__server_name

     func (self TYPE) get_client_name(){
        return self.__client_name

     func (self TYPE) get_session_key(){
        return self._SigningSessionKey

     func (self TYPE) set_session_key(key interface{}){
        self._SigningSessionKey = key

     func (self TYPE) get_encryption_key(){
        if 'Challenge' in self._dialects_data.fields {
            return self._dialects_data["Challenge"]
        } else  {
            return nil

     func (self TYPE) get_server_time(){
     ...
     func (self TYPE) disconnect_tree(tid interface{}){
        smb = NewSMBPacket()
        smb["Tid"]  = tid
     ...
     func (self TYPE) open(tid, filename, open_mode, desired_access interface{}){
     ...
     func (self TYPE) open_andx(tid, filename, open_mode, desired_access interface{}){
        filename = filename.replace('/', '\\')
        filename = filename.encode("utf-16le") if self.__flags2 & SMB.FLAGS2_UNICODE else filename
     ...
     func (self TYPE) close(tid, fid interface{}){
        smb = NewSMBPacket()
        smb["Tid"]    = tid
     ...
     func (self TYPE) send_trans(tid, setup, name, param, data, noAnswer = 0 interface{}){
        smb = NewSMBPacket()
        smb["Tid"]    = tid
     ...
     func (self TYPE) send_trans2(tid, setup, name, param, data interface{}){
        smb = NewSMBPacket()
        smb["Tid"]    = tid
     ...
     func (self TYPE) query_file_info(tid, fid, fileInfoClass = SMB_QUERY_FILE_STANDARD_INFO interface{}){
     ...
     func (self TYPE) __nonraw_retr_file(tid, fid, offset, datasize, callback interface{}){
     ...
     func (self TYPE) __nonraw_stor_file(tid, fid, offset, datasize, callback interface{}){
     ...
     func (self TYPE) get_server_domain(){
        return self.__server_domain

     func (self TYPE) get_server_dns_domain_name(){
        return self.__server_dns_domain_name

     func (self TYPE) get_server_os(){
        return self.__server_os

     func (self TYPE) get_server_os_major(){
        return self.__server_os_major

     func (self TYPE) get_server_os_minor(){
        return self.__server_os_minor

     func (self TYPE) get_server_os_build(){
        return self.__server_os_build

     func (self TYPE) set_server_os(os interface{}){
        self.__server_os = os

     func (self TYPE) get_server_lanman(){
        return self.__server_lanman

     func (self TYPE) is_login_required(){
     ...
     func (self TYPE) is_signing_required(){
        return self._SignatureRequired

     func (self TYPE) get_ntlmv1_response(key interface{}){
        challenge = self._dialects_data["Challenge"]
        return ntlm.get_ntlmv1_response(key, challenge)

     func (self TYPE) kerberos_login(user, password, domain = "", lmhash = "", nthash = "", aesKey = "", kdcHost = "", TGT=nil, TGS=nil interface{}){
     ...
     func (self TYPE) login_extended(user, password, domain = "", lmhash = "", nthash = "", use_ntlmv2 = true  interface{}){
        ...
        if smb.isValidAnswer(SMB.SMB_COM_SESSION_SETUP_ANDX) {
            ...
            if ntlmChallenge["TargetInfoFields_len"] > 0 {
                ...
                if av_pairs[ntlm.NTLMSSP_AV_HOSTNAME] is not nil {
                   try:
                       self.__server_name = av_pairs[ntlm.NTLMSSP_AV_HOSTNAME][1].decode("utf-16le")
                   except UnicodeDecodeError:
                       // For some reason, we couldn't decode Unicode here.. silently discard the operation
                       pass
                if av_pairs[ntlm.NTLMSSP_AV_DOMAINNAME] is not nil {
                   try:
                       ...
                       // For some reason, we couldn't decode Unicode here.. silently discard the operation
                       pass
                if av_pairs[ntlm.NTLMSSP_AV_DNS_DOMAINNAME] is not nil {
                   try:
            ...
            if exportedSessionKey is not nil {
     ...
     func (self TYPE) login(user, password, domain = "", lmhash = "", nthash = "", ntlm_fallback = true interface{}){
     ...
     func (self TYPE) login_standard(user, password, domain = "", lmhash = "", nthash = "" interface{}){
     ...
     func (self TYPE) waitNamedPipe(tid, pipe, timeout = 5, noAnswer = 0 interface{}){
        smb = NewSMBPacket()
        smb["Tid"]    = tid
     ...
     func (self TYPE) read(tid, fid, offset=0, max_size = nil, wait_answer=1 interface{}){
        if not max_size {
            max_size = self._dialects_parameters["MaxBufferSize"] // Read in multiple KB blocks
     ...
     func (self TYPE) read_andx(tid, fid, offset=0, max_size = nil, wait_answer=1, smb_packet=nil interface{}){
        if not max_size {
            if (self._dialects_parameters["Capabilities"] & SMB.CAP_LARGE_READX) and self._SignatureEnabled is false {
                max_size = 65000
            } else  {
     ...
     func (self TYPE) read_raw(tid, fid, offset=0, max_size = nil, wait_answer=1 interface{}){
        if not max_size {
            max_size = self._dialects_parameters["MaxBufferSize"] // Read in multiple KB blocks
     ...
     func write(self,tid,fid,data, offset = 0, wait_answer=1 interface{}){
        smb = NewSMBPacket()
        smb["Tid"]    = tid
     ...
     func write_andx(self,tid,fid,data, offset = 0, wait_answer=1, write_pipe_mode = false, smb_packet=nil interface{}){
        if smb_packet == nil {
            smb = NewSMBPacket()
            smb["Tid"]    = tid
     ...
     func write_raw(self,tid,fid,data, offset = 0, wait_answer=1 interface{}){
     ...
     func (self TYPE) TransactNamedPipe(tid, fid, data = "", noAnswer = 0, waitAnswer = 1, offset = 0 interface{}){
     ...
     func nt_create_andx(self,tid,filename, smb_packet=nil, cmd = nil, shareAccessMode = FILE_SHARE_READ | FILE_SHARE_WRITE, disposition = FILE_OPEN, accessMask = 0x2019f interface{}){
        filename = filename.replace('/', '\\')
        filename = filename.encode("utf-16le") if self.__flags2 & SMB.FLAGS2_UNICODE else filename
     ...
     func (self TYPE) list_path(service, path = "*", password = nil interface{}){
        path = path.replace('/', '\\')
     ...
     func (self TYPE) retr_file(service, filename, callback, mode = FILE_OPEN, offset = 0, password = nil, shareAccessMode = SMB_ACCESS_READ interface{}){
        filename = filename.replace('/', '\\')
     ...
     func (self TYPE) stor_file(service, filename, callback, mode = FILE_OVERWRITE_IF, offset = 0, password = nil, shareAccessMode = SMB_ACCESS_WRITE interface{}){
        filename = filename.replace('/', '\\')
     ...
     func (self TYPE) stor_file_nonraw(service, filename, callback, mode = FILE_OVERWRITE_IF, offset = 0, password = nil, shareAccessMode = SMB_ACCESS_WRITE  interface{}){
        filename = filename.replace('/', '\\')
     ...
     func (self TYPE) check_dir(service, path, password = nil interface{}){
        path = path.replace('/', '\\')
        tid = self.tree_connect_andx('\\\\' + self.__remote_name + '\\' + service, password)
        try:
            smb = NewSMBPacket()
     ...
     func (self TYPE) remove(service, path, password = nil interface{}){
        path = path.replace('/', '\\')
     ...
     func (self TYPE) rmdir(service, path, password = nil interface{}){
        path = path.replace('/', '\\')
     ...
     func (self TYPE) mkdir(service, path, password = nil interface{}){
        path = path.replace('/', '\\')
        tid = self.tree_connect_andx('\\\\' + self.__remote_name + '\\' + service, password)
        try:
            path = path.encode("utf-16le") if self.__flags2 & SMB.FLAGS2_UNICODE else path
     ...
     func (self TYPE) rename(service, old_path, new_path, password = nil interface{}){
     ...
     func (self TYPE) send_nt_trans(tid, subcommand, max_param_count, setup='', param='', data='' interface{}){
        """
     ...
     func (self TYPE) query_sec_info(tid, fid, additional_information=7 interface{}){
        """
        [MS-CIFS]: 2.2.7.6.1
     ...
     func (self TYPE) echo(text = "", count = 1 interface{}){
