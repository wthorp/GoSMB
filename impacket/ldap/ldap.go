...
RE_EX_ATTRIBUTE_2 = re.compile(r'^(){0}%s?%s$' % (DN, MATCHING_RULE), re.I)
 ...
 type LDAPConnection: struct {
     func (self TYPE) __init__(url, baseDN='', dstIp=nil interface{}){
        """
    ...
    def kerberosLogin(self, user, password, domain='', lmhash='', nthash='', aesKey='', kdcHost=nil, TGT=nil,
     ...
     func (self TYPE) login(user='', password='', domain='', lmhash='', nthash='', authenticationChoice='sicilyNegotiate' interface{}){
        """
    ...
    def search(self, searchBase=nil, scope=nil, derefAliases=nil, sizeLimit=0, timeLimit=0, typesOnly=false,
               searchFilter='(objectClass=*)', attributes=nil, searchControls=nil, perRecordCallback=nil):
        if searchBase == nil {
     ...
     func (self TYPE) _handleControls(requestControls, responseControls interface{}){
        done = true
        if requestControls is not nil {
            for requestControl in requestControls:
                if responseControls is not nil {
     ...
     func (self TYPE) close(){
        if self._socket is not nil {
            self._socket.close()

     func (self TYPE) send(request, controls=nil interface{}){
        message = LDAPMessage()
     ...
     func (self TYPE) sendReceive(request, controls=nil interface{}){
        self.send(request, controls)
        return self.recv()

     func (self TYPE) _parseFilter(filterStr interface{}){
        try:
     ...
     func (self TYPE) _consumeCompositeFilter(filterList interface{}){
        try:
            c = filterList.pop()
        except IndexError:
     ...
     func (self TYPE) _consumeSimpleFilter(filterList interface{}){
        try:
            c = filterList.pop()
        except IndexError:
            raise LDAPFilterSyntaxError("EOL while parsing search filter")
    ...
    @staticmethod
     func _compileCompositeFilter(operator, filters interface{}){
        searchFilter = Filter()
        if operator == '!' {
            if len(filters) != 1 {
    ...
    @staticmethod
     func _compileSimpleFilter(attribute, operator, value interface{}){
        searchFilter = Filter()
 ...
 type LDAPFilterSyntaxError struct { // SyntaxError:
    pass


 type LDAPFilterInvalidException struct { // Exception:
    pass


 type LDAPSessionError struct { // Exception:
    """
 ...
 type LDAPSearchError struct { // LDAPSessionError:
     func (self TYPE) __init__(error=0, packet=0, errorString='', answers=nil interface{}){
        LDAPSessionError.__init__(self, error, packet, errorString)
        if answers == nil {
            answers = []
        self.answers = answers

     func (self TYPE) getAnswers(){
        return self.answers
