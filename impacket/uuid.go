...
from struct import pack, unpack
 ...
 func bin_to_string(uuid interface{}){
 ...
 func string_to_bin(uuid interface{}){
 ...
 func stringver_to_bin(s interface{}){
    (maj,min) = s.split(".")
    return pack('<H',int(maj)) + pack('<H',int(min))

 func uuidtup_to_bin(tup interface{}){
    if len(tup) != 2 {
        return
    return string_to_bin(tup[0]) + stringver_to_bin(tup[1])

 func bin_to_uuidtup(bin interface{}){
    assert len(bin) == 20
...
//           "10000000-2000-3000-4000-500000000000" returns ('00000000-0000-0000-0000-000000000000','1.0') 
 func string_to_uuidtup(s interface{}){
 ...
 func uuidtup_to_string(tup interface{}){
    uuid, (maj, min) = tup
