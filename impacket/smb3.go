...
CHANNEL = {
 ...
 type SessionError struct { // Exception:
     ...
     func get_error_packet( self  interface{}){
        return self.packet
 ...
 type SMB3: struct {
    def __init__(self, remote_name, remote_host, my_name=nil, host_type=nmb.TYPE_SERVER, sess_port=445, timeout=60,
                 UDP=0, preferredDialect=nil, session=nil, negSessionResponse=nil):
    ...
    @contextmanager
     func (self TYPE) useTimeout(timeout interface{}){
     ...
     func (self TYPE) signSMB(packet interface{}){
     ...
     func (self TYPE) sendSMB(packet interface{}){
     ...
     func (self TYPE) recvSMB(packetID = nil interface{}){
        ...
        if packet["MessageID"] == packetID or packetID == nil {
     ...
     func (self TYPE) negotiateSession(preferredDialect = nil, negSessionResponse = nil interface{}){
        ...
        if negSessionResponse is not nil {
        ...
        if currentDialect == SMB2_DIALECT_WILDCARD {
            ...
            if preferredDialect is not nil {
     ...
     func (self TYPE) kerberosLogin(user, password, domain = "", lmhash = "", nthash = "", aesKey='', kdcHost = "", TGT=nil, TGS=nil interface{}){
...
//            pass
     ...
     func (self TYPE) login(user, password, domain = "", lmhash = "", nthash = "" interface{}){
        // If we have hashes, normalize them
        if lmhash != '' or nthash != '' {
            if len(lmhash) % 2 {     lmhash = "0%s" % lmhash
            if len(nthash) % 2 {     nthash = "0%s" % nthash
     ...
     func (self TYPE) connectTree(share interface{}){
     ...
     func (self TYPE) create(treeId, fileName, desiredAccess, shareMode, creationOptions, creationDisposition, fileAttributes, impersonationLevel = SMB2_IL_IMPERSONATION, securityFlags = 0, oplockLevel = SMB2_OPLOCK_LEVEL_NONE, createContexts = nil interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) close(treeId, fileId interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) read(treeId, fileId, offset = 0, bytesToRead = 0, waitAnswer = true interface{}){
     ...
     func (self TYPE) write(treeId, fileId, data, offset = 0, bytesToWrite = 0, waitAnswer = true interface{}){
        // IMPORTANT NOTE: As you can see, this was coded as a recursive function
     ...
     func (self TYPE) queryDirectory(treeId, fileId, searchString = "*", resumeIndex = 0, informationClass = FILENAMES_INFORMATION, maxBufferSize = nil, enumRestart = false, singleEntry = false interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if (fileId in self._Session["OpenTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) cancel(packetID interface{}){
        packet = self.SMB_PACKET()
     ...
     func (self TYPE) ioctl(treeId, fileId = nil, ctlCode = -1, flags = 0, inputBlob = "",  maxInputResponse = nil, maxOutputResponse = nil, waitAnswer = 1 interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if fileId == nil {
            fileId = "\xff"*16
     ...
     func flush(self,treeId, fileId interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if (fileId in self._Session["OpenTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) lock(treeId, fileId, locks, lockSequence = 0 interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if (fileId in self._Session["OpenTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) queryInfo(treeId, fileId, inputBlob = "", infoType = SMB2_0_INFO_FILE, fileInfoClass = SMB2_FILE_STANDARD_INFO, additionalInformation = 0, flags = 0  interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if (fileId in self._Session["OpenTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
     ...
     func (self TYPE) setInfo(treeId, fileId, inputBlob = "", infoType = SMB2_0_INFO_FILE, fileInfoClass = SMB2_FILE_STANDARD_INFO, additionalInformation = 0  interface{}){
        if (treeId in self._Session["TreeConnectTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
        if (fileId in self._Session["OpenTable"]) is false {
            raise SessionError(STATUS_INVALID_PARAMETER)
    ...
    // Higher level functions
     ...
     func (self TYPE) mkdir(shareName, pathName, password = nil interface{}){
        // ToDo: Handle situations where share is password protected
     ...
     func (self TYPE) rmdir(shareName, pathName, password = nil interface{}){
        // ToDo: Handle situations where share is password protected
        pathName = pathName.replace('/', '\\')
        pathName = ntpath.normpath(pathName)
        if len(pathName) > 0 and pathName[0] == '\\' {
     ...
     func (self TYPE) remove(shareName, pathName, password = nil interface{}){
        // ToDo: Handle situations where share is password protected
        pathName = pathName.replace('/', '\\')
        pathName = ntpath.normpath(pathName)
        if len(pathName) > 0 and pathName[0] == '\\' {
     ...
     func (self TYPE) retrieveFile(shareName, path, callback, mode = FILE_OPEN, offset = 0, password = nil, shareAccessMode = FILE_SHARE_READ interface{}){
        // ToDo: Handle situations where share is password protected
        path = path.replace('/', '\\')
        path = ntpath.normpath(path)
        if len(path) > 0 and path[0] == '\\' {
     ...
     func (self TYPE) storeFile(shareName, path, callback, mode = FILE_OVERWRITE_IF, offset = 0, password = nil, shareAccessMode = FILE_SHARE_WRITE interface{}){
        // ToDo: Handle situations where share is password protected
        path = path.replace('/', '\\')
        path = ntpath.normpath(path)
        if len(path) > 0 and path[0] == '\\' {
     ...
     func (self TYPE) waitNamedPipe(treeId, pipename, timeout = 5 interface{}){
        ...
        return self.ioctl(treeId, nil, FSCTL_PIPE_WAIT,flags=SMB2_0_IOCTL_IS_FSCTL, inputBlob=pipeWait, maxInputResponse = 0, maxOutputResponse=0)
    ...
    list_path                  = listPath
     ...
     func (self TYPE) nt_create_andx(treeId, fileName, smb_packet=nil, cmd = nil interface{}){
