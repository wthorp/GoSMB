...
// 1.1.1. Initial Token - Checksum field
 type CheckSumField struct { // Structure: (
         Lgth uint32 // =16
         Bnd [6]byte // =b""
         Flags uint32 // =0
    }

 func GSSAPI(cipher interface{}){
...
// 7.2.   GSS-API MIC Semantics
 type GSSAPI_RC4: struct {
    // 1.2.1. Per-message Tokens - MIC
     type MIC struct { // Structure: (
             TOK_ID uint16 // =0x0101
             SGN_ALG uint16 // =0
             Filler uint32 // =0xffffffff
             SND_SEQ [8]byte // =b""
             SGN_CKSUM [8]byte // =b""
        }

    // 1.2.2. Per-message Tokens - Wrap
     type WRAP struct { // Structure: (
             TOK_ID uint16 // =0x0102
             SGN_ALG uint16 // =0
             SEAL_ALG uint16 // =0
             Filler uint16 // =0xffff
             SND_SEQ [8]byte // =b""
             SGN_CKSUM [8]byte // =b""
             Confounder [8]byte // =b""
        }

     func (self TYPE) GSS_GetMIC(sessionKey, data, sequenceNumber, direction = "init" interface{}){
     ...
     func (self TYPE) GSS_Wrap(sessionKey, data, sequenceNumber, direction = "init", encrypt=true, authData=nil interface{}){
     ...
     func (self TYPE) GSS_Unwrap(sessionKey, data, sequenceNumber, direction = "init", encrypt=true, authData=nil interface{}){
        return self.GSS_Wrap(sessionKey, data, sequenceNumber, direction, encrypt, authData)

 type GSSAPI_AES struct { // :
    checkSumProfile = nil
    cipherType = nil
    ...
    // 1.2.2. Per-message Tokens - Wrap
     ...
     func (self TYPE) rotate(data, numBytes interface{}){
        numBytes %= len(data)
        left = len(data) - numBytes
        result = data[left:] + data[:left]
        return result

     func (self TYPE) unrotate(data, numBytes interface{}){
        numBytes %= len(data)
        result = data[numBytes:] + data[:numBytes]
        return result
        
     func (self TYPE) GSS_Wrap(sessionKey, data, sequenceNumber, direction = "init", encrypt=true interface{}){
        token = self.WRAP()
        ...
        cipherText = cipher.encrypt(sessionKey, KG_USAGE_INITIATOR_SEAL,  data + token.getData(), nil)
        token["RRC"] = rrc
     ...
     func (self TYPE) GSS_Unwrap(sessionKey, data, sequenceNumber, direction = "init", encrypt=true, authData=nil interface{}){
        ...
        return plainText[:-(token["EC"]+len(self.WRAP()))], nil

 type GSSAPI_AES256 struct { // GSSAPI_AES:
    checkSumProfile = crypto._SHA1AES256
    cipherType = crypto._AES256CTS

 type GSSAPI_AES128 struct { // GSSAPI_AES:
