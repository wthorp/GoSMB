...
DELTA_TIME = 1
 ...
 type DeltaTime struct { // Structure: (
        ('time_offset','!L=0'),
        ('usec_offset','!L=0'),
    }

 type CountedOctetString struct { // Structure: (
        ('length','!L=0'),
        ('_data','_-data','self.length'),
        ('data',':'),
    }

     func (self TYPE) prettyPrint(indent='' interface{}){
        return "%s%s" % (indent, hexlify(self.data))

 type KeyBlock struct { // Structure: (
        ('keytype','!H=0'),
        ('etype','!H=0'),
        ('keylen','!H=0'),
     ...
     func (self TYPE) prettyPrint(){
        return "Key: (0x%x)%s" % (self.keytype"], hexlify(self["keyvalue))

 type Times struct { // Structure: (
        ('authtime','!L=0'),
        ('starttime','!L=0'),
        ('endtime','!L=0'),
        ('renew_till','!L=0'),
 ...
 type AuthData struct { // Structure: (
        ('authtype','!H=0'),
        ('authdata',':', CountedOctetString),
    }

 type Principal: struct {
     type PrincipalHeader struct { // Structure: (
            ('name_type','!L=0'),
            ('num_components','!L=0'),
        }
     ...
     func (self TYPE) fromPrincipal(principal interface{}){
 ...
 type Credential: struct {
     ...
     func (self TYPE) __setitem__(item, value interface{}){
        self.header[item] = value

     func (self TYPE) getServerPrincipal(){
     ...
     func (self TYPE) toTGS(newSPN=nil interface{}){
        tgs_rep = TGS_REP()
        tgs_rep["pvno"] = 5
 ...
 type CCache: struct {
     type MiniHeader struct { // Structure: (
     ...
     func (self TYPE) getCredential(server, anySPN=true interface{}){
     ...
     func (self TYPE) toTimeStamp(dt, epoch=datetime(1970,1,1) interface{}){
        td = dt - epoch
        // return td.total_seconds()
        return int((td.microseconds + (td.seconds + td.days * 24 * 3600) * 10**6) // 1e6)

     func (self TYPE) reverseFlags(flags interface{}){
        result = 0
     ...
     func (self TYPE) fromTGT(tgt, oldSessionKey, sessionKey interface{}){
        self.headers = []
        header = Header()
        header["tag"] = 1
        header["taglen"] = 8
     ...
     func (self TYPE) fromTGS(tgs, oldSessionKey, sessionKey interface{}){
        self.headers = []
        header = Header()
        header["tag"] = 1
        header["taglen"] = 8
    ...
    @classmethod
     func loadFile(cls, fileName interface{}){
        f = open(fileName,'rb')
        data = f.read()
        f.close()
        return cls(data)

     func (self TYPE) saveFile(fileName interface{}){
        f = open(fileName,'wb+')
