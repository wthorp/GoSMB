...
from impacket.dcerpc.v5.enum import Enum
 ...
 func encodeFlags(flags interface{}){
    finalFlags = list()
 ...
 type ApplicationTagNumbers struct { // Enum:
    Ticket         = 1
    Authenticator  = 2
    EncTicketPart  = 3
    AS_REQ         = 10
 ...
 type PrincipalNameType struct { // Enum:
    NT_UNKNOWN        = 0 
    NT_PRINCIPAL      = 1
    NT_SRV_INST       = 2
    NT_SRV_HST        = 3
 ...
 type PreAuthenticationDataTypes struct { // Enum:
    PA_TGS_REQ                 = 1
    PA_ENC_TIMESTAMP           = 2
    PA_PW_SALT                 = 3
    PA_ENC_UNIX_TIME           = 5
...
// 7.5.4.  Authorization Data Types
 type AuthorizationDataType struct { // Enum:
    AD_IF_RELEVANT                     = 1
...
// 7.5.5.  Transited Encoding Types
 type TransitedEncodingTypes struct { // Enum:
    DOMAIN_X500_COMPRESS = 1

// 7.5.6.  Protocol Version Number
 type ProtocolVersionNumber struct { // Enum:
    pvno = 5

// 7.5.7.  Kerberos Message Types
 type KerberosMessageTypes struct { // Enum:
...
ERROR_MESSAGES = {
 ...
 type APOptions struct { // Enum:
    reserved        = 0
    use_session_key = 1
    mutual_required = 2

 type PAPacOptions struct { // Enum:
    // [MS-KILE] 2.2.10
    claims                                = 0
    branch_aware                          = 1
 ...
 type EncryptionTypes struct { // Enum:
    des_cbc_crc                  = 1
    des_cbc_md4                  = 2
    des_cbc_md5                  = 3
    _reserved_4                  = 4
 ...
 type ChecksumTypes struct { // Enum:
    rsa_md5_des       = 8
    rsa_md4_des       = 4
    hmac_md5          = -138
