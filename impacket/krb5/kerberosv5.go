...
try:
 ...
 func sendReceive(data, host, kdcHost interface{}){
    if kdcHost == nil {
        targetHost = host
    } else  {
        targetHost = kdcHost
 ...
 func getKerberosTGT(clientName, password, domain, lmhash, nthash, aesKey='', kdcHost=nil, requestPAC=true interface{}){
    ...
    for method in methods:
        if method["padata-type"] == constants.PreAuthenticationDataTypes.PA_ETYPE_INFO2.value {
            ...
            for etype2 in etypes2:
                try:
                    if etype2["salt"] == nil or etype2["salt"].hasValue() is false {
                        salt = ""
                    } else  {
            ...
            for etype in etypes:
                try:
                    if etype["salt"] == nil or etype["salt"].hasValue() is false {
                        salt = ""
                    } else  {
    ...
    if nthash != '' and (isinstance(nthash, bytes) and nthash != b'') {
        ...
        key = cipher.string_to_key(password, encryptionTypesData[enctype], nil)
    ...
    if preAuth is true {
        ...
        encriptedTimeStamp = cipher.encrypt(key, 1, encodedTimeStamp, nil)
        ...
        try:
            ...
            if str(e).find("KDC_ERR_ETYPE_NOSUPP") >= 0 {
                if lmhash is '' and nthash is '' and (aesKey is '' or aesKey == nil) {
 ...
 func getKerberosTGS(serverName, domain, kdcHost, tgt, cipher, sessionKey interface{}){
    ...
    // key (Section 5.5.1)
    encryptedEncodedAuthenticator = cipher.encrypt(sessionKey, 7, encodedAuthenticator, nil)
...
// DCE RPC Helpers
//###############################################################################
 func getKerberosType3(cipher, sessionKey, auth_data interface{}){
    ...
    encryptedEncodedAuthenticator = cipher.encrypt(sessionKey, 12, encodedAuthenticator, nil)
 ...
 func getKerberosType1(username, password, domain, lmhash, nthash, aesKey='', TGT = nil, TGS = nil, targetName='', kdcHost = nil, useCache = true interface{}){
    if TGT == nil and TGS == nil {
        if useCache is true {
            try:
 ...
 type SessionKeyDecryptionError struct { // Exception:
    """
    ...
    """
     func __init__( self, message, asRep, cipher, key, cipherText interface{}){
 ...
 type KerberosError struct { // SessionError:
    """
