...
from impacket.ImpactPacket import ProtocolPacket
 ...
 type BootpPacket struct { // ProtocolPacket, structure.Structure:
    commonHdr = (
            ('op','b'),
            ('htype','b=1'),    // 1 = Ether
            ('hlen','b=len(chaddr)'),
 ...
 type DhcpPacket struct { // ProtocolPacket, structure.Structure:
    ...
    options = {
     ...
     func (self TYPE) packOptions(options interface{}){
     ...
     func (self TYPE) getOptionNameAndFormat(optionCode interface{}){
        for k in self.options:
            code,format = self.options[k]
            if code == optionCode { return k, format
        return optionCode, ':'

     func (self TYPE) unpackOptions(options interface{}){
        // options is a string
     ...
     func (self TYPE) unpackParameterRequestList(options interface{}){
        return [self.getOptionNameAndFormat(ord(opt))[0] for opt in options]

     func (self TYPE) isAskingForProxyAutodiscovery(){
     ...
     func (self TYPE) getOptionValue(name interface{}){
        for opt in self.fields["options"]:
            if opt[0] == name {
                return opt[1]
        return nil
