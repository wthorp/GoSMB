...
from impacket.examples.ntlmrelayx.servers.socksserver import activeConnections
 ...
 type HTTPRelayServer struct { // Thread:

     type HTTPServer struct { // socketserver.ThreadingMixIn, socketserver.TCPServer:
         func (self TYPE) __init__(server_address, RequestHandlerClass, config interface{}){
            self.config = config
     ...
     type HTTPHandler struct { // http.server.SimpleHTTPRequestHandler:
         func __init__(self,request, client_address, server interface{}){
            self.server = server
            ...
            if self.server.config.mode != 'REDIRECT' {
                if self.server.config.target == nil {
         ...
         func (self TYPE) handle_one_request(){
            try:
         ...
         func (self TYPE) log_message(format, *args interface{}){
            return

         func (self TYPE) send_error(code, message=nil interface{}){
            if message.find("RPC_OUT") >=0 or message.find("RPC_IN") {
                return self.do_GET()
            return http.server.SimpleHTTPRequestHandler.send_error(self,code,message)

         func (self TYPE) serve_wpad(){
         ...
         func (self TYPE) should_serve_wpad(client interface{}){
         ...
         func (self TYPE) do_HEAD(){
            self.send_response(200)
            self.send_header('Content-type', 'text/html')
            self.end_headers()

         func (self TYPE) do_AUTHHEAD(message = b'', proxy=false interface{}){
            if proxy {
        ...
        //Trickery to get the victim to sign more challenges
         func (self TYPE) do_REDIRECT(proxy=false interface{}){
         ...
         func (self TYPE) do_POST(){
            return self.do_GET()

         func (self TYPE) do_CONNECT(){
            return self.do_GET()

         func (self TYPE) do_GET(){
            messageType = 0
            if self.server.config.mode == 'REDIRECT' {
                self.do_SMBREDIRECT()
                return
            ...
            if (proxy and proxyAuthHeader == nil) or (not proxy and autorizationHeader == nil) {
            ...
            if messageType == 1 {
                ...
                if not self.do_ntlm_auth(token,authenticateMessage) {
                    ...
                    if self.server.config.outputFile is not nil {
         ...
         func (self TYPE) do_ntlm_negotiate(token, proxy interface{}){
         ...
         func do_ntlm_auth(self,token,authenticateMessage interface{}){
     ...
     func (self TYPE) __init__(config interface{}){
