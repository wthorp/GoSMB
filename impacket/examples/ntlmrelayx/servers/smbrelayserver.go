...
from impacket.dcerpc.v5.rpcrt import DCERPCException
 ...
 type SMBRelayServer struct { // Thread:
     func __init__(self,config interface{}){
        ...
        if self.config.outputFile is not nil {
    ...
    //## SMBv2 Part #################################################################
     func (self TYPE) SmbNegotiate(connId, smbServer, recvPacket, isSMB1=false interface{}){
     ...
     func (self TYPE) SmbSessionSetup(connId, smbServer, recvPacket interface{}){
        connData = smbServer.getConnectionData(connId, checkStatus = false)
    ...
    //## SMBv1 Part #################################################################
     func (self TYPE) SmbComNegotiate(connId, smbServer, SMBCommand, recvPacket interface{}){
        connData = smbServer.getConnectionData(connId, checkStatus = false)
        if self.config.mode.upper() == 'REFLECTION' {
            self.targetprocessor = TargetsProcessor(singleTarget='SMB://%s:445/' % connData["ClientIP"])
     ...
     func (self TYPE) SmbSessionSetupAndX(connId, smbServer, SMBCommand, recvPacket interface{}){
    ...
    //Initialize the correct client for the relay target
     func init_client(self,extSec interface{}){
     ...
     func do_ntlm_negotiate(self,client,token interface{}){
     ...
     func do_ntlm_auth(self,client,SPNEGO_token,challenge interface{}){
     ...
     func do_attack(self,client interface{}){
