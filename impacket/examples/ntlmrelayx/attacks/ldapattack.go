...
delegatePerformed = []
 type LDAPAttack struct { // ProtocolAttack:
    """
    ...
    GENERIC_ALL             = 0x000F01FF
     ...
     func (self TYPE) __init__(config, LDAPClient, username interface{}){
        ProtocolAttack.__init__(self, config, LDAPClient, username)

     func (self TYPE) addComputer(parent, domainDumper interface{}){
        """
     ...
     func (self TYPE) addUser(parent, domainDumper interface{}){
        """
     ...
     func (self TYPE) addUserToGroup(userDn, domainDumper, groupDn interface{}){
        global alreadyEscalated
        // For display only
     ...
     func (self TYPE) delegateAttack(usersam, targetsam, domainDumper interface{}){
     ...
     func (self TYPE) aclAttack(userDn, domainDumper interface{}){
        global alreadyEscalated
        if alreadyEscalated {
     ...
     func (self TYPE) writeRestoreData(restoredata, domaindn interface{}){
        output = {}
        domain = re.sub(',DC=', '.', domaindn[domaindn.find("DC="):], flags=re.I)[3:]
     ...
     func (self TYPE) validatePrivileges(uname, domainDumper interface{}){
        // Find the user's DN
        membersids = []
        sidmapping = {}
        privs = {
            'create': false, // Whether we can create users
            'createIn': nil, // Where we can create users
            'escalateViaGroup': false, // Whether we can escalate via a group
            'escalateGroup': nil, // The group we can escalate via
            'aclEscalate': false, // Whether we can escalate via ACL on the domain object
            'aclEscalateIn': nil // The object which ACL we can edit
        }
     ...
     func (self TYPE) getUserInfo(domainDumper, samname interface{}){
     ...
     func (self TYPE) checkSecurityDescriptors(entries, privs, membersids, sidmapping, domainDumper interface{}){
        standardrights = [
            self.GENERIC_ALL,
            self.GENERIC_WRITE,
            self.GENERIC_READ,
    ...
    @staticmethod
     func aceApplies(ace_guid, object_class interface{}){
        '''
...
// Create an object ACE with the specified privguid and our sid
 func create_object_ace(privguid, sid interface{}){
    nace = ldaptypes.ACE()
...
// Create an ALLOW ACE with the specified sid
 func create_allow_ace(sid interface{}){
    nace = ldaptypes.ACE()
...
// Check if an ACE allows for creation of users
 func can_create_users(ace interface{}){
...
// Check if an ACE allows for adding members
 func can_add_member(ace interface{}){
