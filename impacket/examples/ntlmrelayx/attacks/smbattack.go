...
PROTOCOL_ATTACK_CLASS = "SMBAttack"
 ...
 type SMBAttack struct { // ProtocolAttack:
    """
    ...
    """
    PLUGIN_NAMES = ["SMB"]
     func (self TYPE) __init__(config, SMBClient, username interface{}){
        ...
        if self.config.interactive {
            ...
            self.tcpshell = nil
            if self.config.exeFile is not nil {
     ...
     func (self TYPE) run(){
        ...
        if self.config.exeFile is not nil {
            ...
            try:
                if self.config.command is not nil {
                ...
                if samHashes is not nil {
                    samHashes.finish()
                if remoteOps is not nil {
                    remoteOps.finish()
